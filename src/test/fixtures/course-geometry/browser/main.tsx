...
createRoot(document.getElementById('root')!).render(params.has('onetap') ? <OneTapFixture course={compiledCourse ?? 'cacapon'} holeNumber={Number(params.get('hole') ?? 7)} showBar={params.has('bar')} playMode={params.get('mode') === 'competition' ? 'competition' : 'practice'} world={params.get('world') === 'v2' ? 'v2' : undefined} /> : params.has('lab') ? <MeridianLabFixture course={compiledCourse ?? 'cacapon'} /> : params.has('play') ? <PlayScreen /> : params.has('matrix') ? <CourseMatrixFixture course={compiledCourse ?? 'cacapon'} holeNumber={Number(params.get('hole') ?? 7)} /> : sourceStudy === 'bryan' || sourceStudy === 'cardinal'
