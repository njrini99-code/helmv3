...
export function CourseMatrixFixture({ course, holeNumber }: { course: CompiledCourse; holeNumber: number }) {
  ...
  // The matrix is the canary source: it must see the same context layer the player does.
  const scene = buildHoleScene(pkg, hole.key, [], mesh, contextLayerFor(course) ?? undefined);
