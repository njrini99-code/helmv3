...
const SCRIPTS = [
  'scripts/markdown-lint-ratchet.mjs',
  'scripts/check-doc-path-drift.mjs',
];
...
describe('doc ratchets are scoped to tracked files', () => {
  ...
  it('markdown-lint-ratchet does not lint the bare docs/ walk', () => {
    const src = readFileSync(resolve(process.cwd(), SCRIPTS[0]), 'utf8');
  ...
  it('check-doc-path-drift filters inside its walk, not after', () => {
    const src = readFileSync(resolve(process.cwd(), SCRIPTS[1]), 'utf8');
