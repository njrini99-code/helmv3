...
export const FLAG_REGISTRY: readonly FlagDefinition[] = [
  ...
  },
  {
    ...
    owner: "golf/coachhelm",
    purpose: "Gates whether the Game Fingerprint page's Approach section renders the new distance-profile surface (buildRollingDistanceProfileScope + loadDistanceProfile + DistanceProfileSection) alongside the existing stats-cache-backed KPIs; default off pending a design/product review of the new section's placement and copy. ON since 2026-09-25 by owner decision; its migrations are applied in production (checked 2026-09-25).",
    type: "experiment",
    status: "active",
    created_at: "2026-09-23",
    expires_at: null,
  ...
  },
  {
    ...
    owner: "golf/coachhelm",
    purpose: "Gates the coach-written \"done\" criteria and the practice-completion log rows for a focus area, added for A8 slice 2. Off means zero reads or writes of either new table anywhere in the app — required because the migration that creates them has not been applied in every environment yet. ON since 2026-09-25 by owner decision; its migrations are applied in production (checked 2026-09-25).",
    type: "temporary_migration",
    status: "active",
    created_at: "2026-09-23",
  ...
  },
  {
    ...
    kill_switch_behavior: null,
    cleanup_plan: "Flip on (development, then preview, then production) only after the owner applies 20260923100000_round_recap_single_flight_lock to each environment, confirmed via information_schema.columns (ai_narrative) and pg_proc (claim_round_recap_lock / release_round_recap_lock). Once on everywhere and the fallback rate (deterministic vs. LLM-authored) is acceptable, consider promoting to a permanent `release` flag — owner's call, not automatic.",
  },
  {
  ...
  },
  {
    ...
    owner: "golf/coachhelm",
    purpose: "Adjusts a coach's v2 alert-generation thresholds (decline, pressure gap) using their own ack/dismiss history instead of only the coach-set CoachPhilosophy values; default off pending real-world evidence that this improves alert relevance. ON since 2026-09-25 by owner decision; its migrations are applied in production (checked 2026-09-25).",
    type: "experiment",
    status: "active",
    created_at: "2026-09-23",
    expires_at: null,
