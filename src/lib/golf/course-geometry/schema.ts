...
const packageSchema = z.object({
  ...
  contentHash: z.string().regex(/^[a-f0-9]{64}$/), status: z.enum(['reviewed_draft', 'source_candidate']),
...
export function parseGeometryPackage(input: unknown): CourseGeometryPackage {
  ...
  for (const h of pkg.holes) {
    ...
    if (h.routeFeatureId && (!route || !h.featureIds.includes(route.id))) throw new Error(`Missing route: ${h.key}`);
    if (!h.routeFeatureId && (pkg.status !== 'source_candidate' || h.completeness !== 'partial' || !h.displayLabel || !h.greenFeatureId)) throw new Error(`Unlabelled unrouted study: ${h.key}`);
