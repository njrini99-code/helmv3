// =============================================================================
// Unit tests for player-passport.ts's summarizePitchingSeason — the #434
// follow-up wiring the shared thirds-aware IP helper (src/lib/baseball/innings.ts)
// into the season pitching summary. IP is recorded in baseball's "outs"
// notation (.1/.2 = thirds of an inning, not a base-10 fraction), so summing
// with plain `+` is wrong: 5.2 + 3.2 must sum to 9.1 (28 outs), not the naive
// 8.4, and ERA/WHIP must divide by TRUE innings (outs / 3), not the notation
// sum. PURE function; no DB/mocking needed.
// =============================================================================

import { describe, it, expect } from 'vitest';

import { summarizePitchingSeason } from '../player-passport';

describe('player-passport summarizePitchingSeason', () => {
  ...
  it('derives ERA/WHIP from TRUE innings (28 outs / 3 = 9.333…), not the 9.1 notation value', () => {
    ...
    // Confirm it's NOT dividing by the notation value (9.1) — the two diverge.
  ...
  it('the classic #434 regression case: 6.1 + 6.2 -> 13.0, not 12.3', () => {
    ...
    // 39 outs = 13 true innings exactly, so era/whip match a plain /13 here.
