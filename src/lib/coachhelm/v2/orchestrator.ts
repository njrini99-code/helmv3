...
import { ApproachMissCompassGenerator } from '@/lib/coachhelm/v3/generators/insight-angles/approach-miss-compass';
...
class CoachHelmIntelligence {
  ...
  async analyzePlayer(
    ...
    const tier1Generators: Array<{ name: string; fn: () => Promise<unknown> }> = !runInsightGenerators ? [] : [
      ...
      // v3 — insight angles (2026-09-25; all five behind the
      ...
      { name: 'v3.approachMissCompass', fn: () => new ApproachMissCompassGenerator(playerId).run() },
