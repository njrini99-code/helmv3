 ...
 *
 * The fake Supabase client below implements `golf_insight_action` and
 * `golf_insight_exposure` — the two cheap anchor-resolution reads
 * `resolveInterventionAnchor` always makes, action first, exposure as its
 * fallback (Package 10 owner decision: prefer the first genuine action over
 * the first exposure; see comparable-attribute.ts's doc comment) — and
...
describe('computeComparableAttribution — real core, open-window short-circuit', () => {
  it('never reaches the real loadPlayerContext/computeComparableOpportunities when the follow-up window is still open', async () => {
    ...
    try {
      ...
      // No qualifying action on record — resolveInterventionAnchor falls
      // back to the exposure row below, exactly like a real never-acted-on
      // insight.
      const actionBuilder = {
      ...
      // Only the two cheap anchor-resolution lookups ran — nothing else was
      // ever queried.
      expect(client.from).toHaveBeenCalledTimes(2);
      expect(client.from).toHaveBeenCalledWith('golf_insight_action');
