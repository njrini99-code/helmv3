...
import { isSafePushEndpoint, isSafePushEndpointShape } from '@/lib/security/ssrf';

// The SSRF guard used to live here AND inline in
// src/app/api/push-subscriptions/route.ts — two hand-copied string-only
// copies, so neither resolved a hostname and a fix to one left the other
// open. Both now import the single implementation in @/lib/security/ssrf.
//
// Re-exported because the existing verdict tests assert the string-only
// judgements synchronously; the DNS-resolving check is used on the send path.
export { isSafePushEndpointShape };
...
export async function sendWebPush(
  ...
  // Re-validated at send time, not only at store time: a row could predate
  // this guard, and a name's records can change after it was accepted. This
  // is the check immediately before the server opens the connection.
