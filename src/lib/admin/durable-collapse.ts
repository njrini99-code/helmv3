...
export interface DurableCollapseOutcome {
  ...
  eventId: string | null;
  /** Why the collapse did not happen, when it did not. `null` = nothing to collapse into. */
  reason: 'no_recent_row' | 'lookup_failed' | 'update_failed' | null;
}
...
export async function absorbIntoRecentEvent(
  ...
  try {
    ...
    if (lookup.error) return { collapsed: false, eventId: null, reason: 'lookup_failed' };
    const row = lookup.data?.[0];
    if (!row) return { collapsed: false, eventId: null, reason: 'no_recent_row' };

    const metadata = bumpCollapsedMetadata(row.metadata, { by: opts.by ?? 1, at: now.toISOString() });
    const update = await admin
      .from('admin_events')
      .update({ metadata: metadata as Json })
      .eq('id', row.id);
    if (update.error) return { collapsed: false, eventId: null, reason: 'update_failed' };

    return { collapsed: true, eventId: row.id, reason: null };
  } catch {
