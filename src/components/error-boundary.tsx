...
/**
 * Inline Error Fallback for Dashboard Sections
 * Compact fallback that doesn't disrupt layout when one section fails
 */
export function SectionErrorFallback({ error, resetError }: ErrorFallbackProps) {
  return (
    <div className="flex flex-col items-center justify-center py-8 px-4 text-center bg-red-50/50 rounded-xl border border-red-100">
      ...
      {process.env.NODE_ENV === 'development' && error && (
        <p className="text-xs text-red-500 mb-2 font-mono max-w-xs truncate">
          {error.message}
        </p>
      )}
