...
function DevPlanCard({ item }: { item: DevPlanProgressItem }) {
  return (
    <Link 
      ...
      className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 bg-white hover:border-slate-200 hover:shadow-sm transition-all group"
    >
      <Avatar 
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-slate-900 truncate">{item.playerName}</p>
          {item.hasOverdue && (
      ...
      <div className="flex items-center gap-2">
        <span className="text-lg font-semibold text-slate-900 tabular-nums">{item.progressPct}%</span>
        <IconChevronRight 
          size={16} 
    ...
    <div className="relative glass-standard rounded-2xl overflow-clip">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100/50">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-slate-900">Dev Plan Progress</h3>
        </div>
        <Link 
      ...
      <div className="p-4">
        {data.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <h4 className="text-sm font-medium text-slate-900 mb-1">No active dev plans</h4>
