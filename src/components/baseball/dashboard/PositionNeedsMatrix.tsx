    ...
    <div className="relative glass-standard rounded-2xl overflow-clip">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-slate-900 tracking-tight">Position Needs</h2>
            <p className="text-xs text-slate-500">Class of {gradYear}</p>
          </div>
        </div>
        {onSetGoals && (
          <Button
      ...
      {/* Summary bar */}
      <div className="px-6 py-3 bg-slate-50/50 border-b border-slate-100/50">
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-4">
            <span className="text-slate-600">
              <span className="font-semibold text-slate-900">{summary.totalCommitted}</span>
              <span className="text-slate-400"> / </span>
              <span>{summary.totalNeed}</span>
              <span className="text-slate-400 ml-1">filled</span>
            </span>
            <span className="text-slate-300">|</span>
            <span className="text-slate-600">
              <span className="font-medium text-slate-700">{summary.totalPipeline}</span>
              <span className="text-slate-400 ml-1">in pipeline</span>
            </span>
          </div>
        ...
        <p className="text-xs text-slate-400 mt-1">{summary.percentFilled}% of class filled</p>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100/50 bg-slate-50/30">
              ...
              <th className="px-6 py-2.5 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">
                Status
              </th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="text-sm text-slate-600 tabular-nums">{pos.need}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={cn(
                      "text-sm font-medium tabular-nums",
                      pos.pipeline === 0 ? "text-slate-400" : "text-slate-700"
                    )}>
                      {pos.pipeline}
                    </span>
                  </td>
