    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-3">
          {events.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-6 text-center">
              <IconCalendar size={24} className="text-slate-300 mb-2" />
              ...
              {events.slice(0, 4).map(event => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{formatEventDate(event.startTime)}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <div className="relative glass-standard rounded-2xl overflow-clip">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-900 text-sm">Tasks Due</h3>
            {pendingTasks > 0 && (
        ...
        <div className="p-3">
          {pendingTasks === 0 ? (
            <div className="flex flex-col items-center justify-center py-6 text-center">
              <IconClipboardList size={24} className="text-slate-300 mb-2" />
              <p className="text-xs text-slate-500">No pending tasks</p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-6">
              <p className="text-3xl font-bold text-slate-900 tabular-nums">{pendingTasks}</p>
              <p className="text-xs text-slate-500 mt-1">tasks pending</p>
      ...
      <div className="relative glass-standard rounded-2xl overflow-clip">
        ...
        <div className="p-3">
          {unreadMessages === 0 ? (
            <div className="flex flex-col items-center justify-center py-6 text-center">
              <IconBell size={24} className="text-slate-300 mb-2" />
              <p className="text-xs text-slate-500">All caught up!</p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-6">
              <p className="text-3xl font-bold text-slate-900 tabular-nums">{unreadMessages}</p>
