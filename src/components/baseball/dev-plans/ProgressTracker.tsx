  ...
  return (
    <div className="rounded-xl border border-warm-200 bg-cream-100/75 p-4">
      <div className="flex items-center justify-between text-sm text-warm-600">
        ...
        <span className="font-medium text-warm-900">{percent}%</span>
      </div>
      <div className="mt-3 h-2 rounded-full bg-warm-200">
        <div
          className={cn('h-2 rounded-full', percent >= 70 ? 'bg-primary-600' : percent >= 40 ? 'bg-amber-500' : 'bg-warm-400')}
          style={{ width: `${percent}%` }}
        />
      </div>
