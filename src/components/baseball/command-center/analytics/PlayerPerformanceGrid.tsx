...
function getHeatmapColor(
  ...
  if (value == null) return 'bg-warm-100 text-warm-400';
...
export function PlayerPerformanceGrid({ players }: PlayerPerformanceGridProps) {
  ...
  return (
    <div className="bg-cream-100/75 backdrop-blur-xl border border-white/20 rounded-2xl overflow-clip">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-warm-50/50">
              <th className="px-4 py-3 text-left font-medium text-warm-600 sticky left-0 bg-warm-50/50">
                Player
              </th>
              {METRICS.map((metric) => (
                <th
                  key={metric.key}
                  className="px-3 py-3 text-center font-medium text-warm-600 cursor-pointer hover:bg-warm-100 transition-colors whitespace-nowrap"
      ...
      {players.length > 15 && (
        <div className="p-3 text-center border-t border-warm-100">
      ...
      <div className="p-4 border-t border-warm-100 bg-warm-50/30">
        <div className="flex items-center justify-center gap-4 text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-warm-600">Excellent</span>
          </div>
          <div className="flex items-center gap-1.5">
      ...
      <div className="p-4">
        <div className="space-y-2">
          ...
          <div className="flex gap-3">
            <div className="w-32 h-6 bg-warm-200 rounded animate-pulse" />
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="w-12 h-6 bg-warm-200 rounded animate-pulse" />
            ))}
          </div>
          {/* Rows */}
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="flex gap-3">
              <div className="w-32 h-8 bg-warm-100 rounded animate-pulse" />
              {Array.from({ length: 6 }).map((_, j) => (
                <div key={j} className="w-12 h-8 bg-warm-100 rounded animate-pulse" />
              ))}
            </div>
          ))}
        </div>
