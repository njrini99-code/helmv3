          ...
          {game.our_score != null && game.opponent_score != null && (
            <div className="flex items-center gap-3">
              ...
              <div className="text-center">
                <p className="text-xs text-slate-400 font-medium">FINAL</p>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60">
                  <th className="text-left px-4 py-2.5 font-semibold text-slate-500 sticky left-0 bg-slate-50/80 min-w-[140px]">Player</th>
                  {['AB','R','H','2B','3B','HR','RBI','BB','K','SB','HBP'].map((h) => (
                    <th key={h} className="text-center px-2 py-2.5 font-semibold text-slate-500 min-w-[36px]">{h}</th>
                  ))}
                  <th className="text-center px-3 py-2.5 font-semibold text-slate-400 min-w-[48px]">AVG</th>
                  <th className="text-center px-3 py-2.5 font-semibold text-slate-400 min-w-[48px]">OBP</th>
                  <th className="text-center px-3 py-2.5 font-semibold text-slate-400 min-w-[48px]">SLG</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {batting.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      {row.player ? (
                        <span>
                          ...
                          {row.player.primary_position && (
                            <span className="ml-1 text-[10px] text-slate-400">{row.player.primary_position}</span>
                          )}
                        </span>
                      ) : '—'}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-center font-mono text-slate-700">{fmtAvg(row.avg)}</td>
                    <td className="px-3 py-2.5 text-center font-mono text-slate-500">{fmtAvg(row.obp)}</td>
                    <td className="px-3 py-2.5 text-center font-mono text-slate-500">{fmtAvg(row.slg)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-slate-200 bg-slate-50/80 font-semibold text-slate-700">
                  <td className="px-4 py-2.5 sticky left-0 bg-slate-50/90 text-xs font-bold uppercase tracking-wide">TOTALS</td>
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white/20 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60">
                  <th className="text-left px-4 py-2.5 font-semibold text-slate-500 sticky left-0 bg-slate-50/80 min-w-[140px]">Pitcher</th>
                  {['IP','H','R','ER','BB','K','HR','PC'].map((h) => (
                    <th key={h} className="text-center px-2 py-2.5 font-semibold text-slate-500 min-w-[40px]">{h}</th>
                  ))}
                  <th className="text-center px-3 py-2.5 font-semibold text-slate-400 min-w-[52px]">ERA</th>
                  <th className="text-center px-3 py-2.5 font-semibold text-slate-400 min-w-[52px]">WHIP</th>
                  <th className="text-center px-3 py-2.5 font-semibold text-slate-400 min-w-[40px]">Dec</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {pitching.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50/40 transition-colors">
                    ...
                    <td className="px-2 py-2.5 text-center tabular-nums">{fmtStat(row.hr)}</td>
                    <td className="px-2 py-2.5 text-center tabular-nums text-slate-500">{fmtStat(row.pitch_count)}</td>
                    <td className="px-3 py-2.5 text-center font-mono text-slate-700">{fmtStat(row.era, 2)}</td>
                    <td className="px-3 py-2.5 text-center font-mono text-slate-500">{fmtStat(row.whip, 3)}</td>
