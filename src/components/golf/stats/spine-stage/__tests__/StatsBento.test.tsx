...
describe('StatsBento — Standing cell chip/headline pairing', () => {
  ...
  it('labels the weakest category as a leak, never as the player\u2019s best', () => {
    ...
    const cell = headline.closest('[data-slot="bento-cell"]') ?? headline.parentElement!;
    // The headline names the WEAKEST category, so the chip beside it has to
    // read "Leak". Pairing it with "Best" claimed the leak was the strength.
    expect(cell.textContent).toContain('Leak');
    expect(cell.textContent).not.toContain('Best');
  });
  ...
  it('falls back to the strongest category when there is no weakness to report', () => {
    ...
    const cell = headline.closest('[data-slot="bento-cell"]') ?? headline.parentElement!;
    expect(cell.textContent).toContain('Best');
  });
});
