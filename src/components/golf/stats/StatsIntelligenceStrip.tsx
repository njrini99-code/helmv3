...
function TrendBadge({ trend }: { trend: PlayerStatsIntelligence['trend'] }) {
  ...
  return (
    ...
    >
      <Icon size={12} />
      {meta.label}
      {deltaLabel && <span className="text-caption opacity-80">{deltaLabel}</span>}
    </span>
  );
}
    ...
    <div className="surface-matte rounded-3xl p-5 md:p-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {compositeDisplay != null && (
            ...
            >
              {compositeDisplay}
              <span className="text-caption font-medium text-warm-400">/100</span>
            </span>
          )}
      ...
      {hasRatings && data?.categories && (
        <div className="space-y-2.5">
          ...
          {sampleTooSmall && (
            <p className="text-caption text-warm-400 pt-1">
      ...
      {hasInsights && topInsightRow && (
        <div className="space-y-2 pt-1">
          <div className="flex items-center justify-between">
            <p className="text-caption font-medium uppercase tracking-[0.12em] opacity-80 text-warm-500">
              Top insight
            </p>
            <Link
