...
export function OneTapReadout({ view }: { view: OneTapView }) {
  ...
  return <div className="flex items-end justify-between gap-3 pb-2" data-slot="one-tap-distances" data-basis={distancesBasis ?? ''} data-readout-mode={readout?.mode ?? ''} data-play-mode={policy.mode}>
    ...
    </div> : <p className="text-caption text-text-secondary" data-slot="one-tap-no-readout" data-quality={view.locationQuality}>{!view.hasGreen ? 'No mapped green on this hole'
      : view.locationQuality === 'approximate' ? 'Approximate location only · turn on Precise Location for this app in your phone\u2019s Settings'
        : view.locationQuality === 'off_course' ? 'Distances start when you reach the course' : 'Waiting for a GPS fix'}</p>}
