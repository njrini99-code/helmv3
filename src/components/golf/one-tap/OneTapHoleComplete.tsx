...
export function OneTapHoleComplete({ round }: { round: OneTapRoundView }) {
  ...
  return <CompletionCard key={`${completion.holeKey}:${completion.inferred}`} completion={completion} round={round} />;
}
...
function CompletionCard({ completion, round }: { completion: OneTapHoleCompletion; round: OneTapRoundView }) {
  ...
  if (completion.clean) {
    return <div className={`${card} text-caption font-semibold text-text-primary`} style={top} role="status" data-slot="one-tap-hole-complete" data-integrity="CLEAN" data-hole-key={completion.holeKey}>
      Hole {completion.ordinal} · {shotsLabel(completion.shots)} · complete ✓
    </div>;
  }
  return <div className={card} style={top} role="status" data-slot="one-tap-hole-complete" data-integrity={completion.report.integrity} data-inferred={completion.inferred ? 'true' : undefined} data-hole-key={completion.holeKey}>
    <div className="flex items-center justify-between gap-2">
      <span className="text-caption font-semibold text-text-primary">Hole {completion.ordinal} · check {flags.length} {flags.length === 1 ? 'item' : 'items'}</span>
      <Button variant="ghost" size="sm" onClick={() => setReviewing(r => !r)} aria-expanded={reviewing} data-slot="one-tap-review">Review</Button>
    </div>
    {reviewing && <div className="mt-2 flex flex-col gap-2 border-t border-border-subtle pt-2" data-slot="one-tap-review-panel">
      ...
      <div className="flex items-center gap-2">
        <Button variant="secondary" size="sm" onClick={round.returnToCompleted} data-slot="one-tap-review-back">{completion.inferred ? `Back to hole ${completion.ordinal}` : 'Reopen hole'}</Button>
        <Button variant="ghost" size="sm" onClick={round.dismissCompletion} data-slot="one-tap-review-keep">Keep as is</Button>
      </div>
    </div>}
  </div>;
}
