...
export function PredictionCard({ prediction, playerName }: PredictionCardProps) {
  ...
  const getTrendIcon = () => {
    if (isNeutral) return <IconTarget size={18} className="text-slate-400" />;
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-slate-50 to-white rounded-xl border border-slate-200 p-4 hover:shadow-md transition-shadow"
    >
      {/* Header */}
      ...
      {isTailRiskProbabilities(prediction.tailRisks) && (
        <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-slate-100">
          <div className="text-center">
            <div className="text-xs text-slate-400">Blowup Risk</div>
            <div className="text-sm font-medium text-red-500">
              {Math.round(prediction.tailRisks.blowupProbability * 100)}%
            </div>
          </div>
          <div className="text-center">
            <div className="text-xs text-slate-400">Great Round</div>
