        ...
        {patterns.map((pattern, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span>
                Confidence: <span className="font-medium text-slate-700">{Math.round(pattern.confidence * 100)}%</span>
              </span>
              <span>
                Occurrences: <span className="font-medium text-slate-700">{pattern.occurrenceCount}</span>
              </span>
              <span className={cn(
                'px-1.5 py-0.5 rounded text-xs font-medium',
                pattern.trend === 'strengthening' && 'bg-amber-100 text-amber-700',
                pattern.trend === 'stable' && 'bg-slate-100 text-slate-600',
