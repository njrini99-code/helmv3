  ...
  if (!history || history.length === 0) {
    return (
      <div className={cn('text-center py-8', className)}>
        ...
        <p className="text-sm text-warm-500">No status history available</p>
      </div>
    );
  }
...
function TimelineEntry({
  ...
  return (
    <div className="relative flex gap-4 pb-6">
      {/* Timeline line */}
      {!isLast && (
        <div className="absolute left-[15px] top-[32px] bottom-0 w-0.5 bg-warm-200" />
      )}
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        ...
        <div className="flex items-start justify-between gap-4 mb-2">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 text-xs text-warm-500">
              ...
              <span className="font-medium text-warm-700">{entry.changed_by_name}</span>
              <span>•</span>
        ...
        {/* Reason (if provided) */}
        {entry.reason && (
          <div className="flex items-start gap-2 p-3 bg-warm-50 rounded-lg mt-2">
            <MessageSquare className="w-4 h-4 text-warm-400 shrink-0 mt-0.5" />
            <p className="text-sm text-warm-700 italic">&ldquo;{entry.reason}&rdquo;</p>
          </div>
        )}
...
function StatusChangeDescription({ entry }: { entry: StatusHistoryEntry }) {
  ...
  if (isCreation) {
    return <span className="text-sm text-warm-900">Created event as draft</span>;
  }
  ...
  return <span className="text-sm text-warm-900">{description}</span>;
}
      ...
      {sortedHistory.map((entry) => {
        ...
        return (
          <div key={entry.id} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-warm-600">{relativeTime}</span>
          </div>
        );
      })}
    </div>
