...
/**
 * Compact RSVP buttons for inline use (e.g., in event cards)
 */
export function CompactMobileRSVPButtons({
  currentResponse,
  onRespond,
  disabled = false,
  className,
}: Omit<MobileRSVPButtonsProps, 'size' | 'layout' | 'showLabels'>) {
  return (
    <MobileRSVPButtons
...
/**
 * Icon-only RSVP buttons for very compact spaces
 */
export function IconOnlyRSVPButtons({
  currentResponse,
  onRespond,
  disabled = false,
}: Pick<MobileRSVPButtonsProps, 'currentResponse' | 'onRespond' | 'disabled'>) {
  const [loadingResponse, setLoadingResponse] = useState<RSVPResponse | null>(null);
  const { triggerHaptic } = useHapticFeedback();
      ...
      {RSVP_OPTIONS.map((option) => {
        ...
        return (
          <button
            ...
            className={cn(
              'p-2 rounded-lg transition-all min-w-[44px] min-h-[44px]',
              'flex items-center justify-center',
              'touch-manipulation',
              'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500',
              isActive
                ? option.activeClass
                : cn(option.inactiveClass, option.hoverClass),
              disabled && 'opacity-50 cursor-not-allowed',
              'active:scale-95'
            )}
            title={option.label}
