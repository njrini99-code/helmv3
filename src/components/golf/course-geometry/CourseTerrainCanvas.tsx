...
export function CourseTerrainCanvas({ scene, mesh, camera, width, height, fallback, onUnavailable, selectedShotNumber, runtimeRef, debugView, world, visualArtifact, styleOverrides, quality, markers, reservedRects }: {
  ...
  const latest = useRef({ scene, camera, width, height, selectedShotNumber, markers, reservedRects });
  ...
  useLayoutEffect(() => {
    latest.current = { scene, camera, width, height, selectedShotNumber, markers, reservedRects };
    onFailure.current = onUnavailable;
    const active = runtime.current;
    if (!active) return;
    active.setEvidence(scene, selectedShotNumber);
    active.setMarkers(markers ?? null);
    active.setReservedRects(reservedRects ?? []);
    active.setCamera(camera, width, height);
  }, [scene, selectedShotNumber, camera, width, height, onUnavailable, markers, reservedRects]);
  ...
  useEffect(() => {
    ...
    void import('@/lib/golf/course-geometry/three-renderer').then(async module => {
      ...
      owned.setReservedRects(latest.current.reservedRects ?? []);
      await owned.ready;
