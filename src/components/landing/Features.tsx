...
function FeatureCarousel({ 
  ...
  const featureProgress = (progress - 0.25 - (activeIndex * 0.15)) / 0.15
  const clampedProgress = Math.max(0, Math.min(1, featureProgress))
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-6">
      ...
      >
        ...
        {/* Content */}
        <div 
          className="relative h-[320px] bg-gradient-to-br from-stone-900 via-stone-800 to-stone-900"
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              <div className="text-center">
                ...
                {/* Description */}
                <motion.p
                  className="text-base md:text-lg text-stone-400 max-w-md mx-auto leading-relaxed"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
      ...
      <div className="mt-8 text-center">
        <AnimatePresence mode="wait">
          ...
          >
            <h4 className="text-2xl md:text-3xl font-semibold text-stone-900 mb-2">
              {features[activeIndex]?.title}
            </h4>
...
export function Features() {
  ...
  // Previous index for direction calculation
  const prevIndexRef = useRef(0)
  
  // Update phases and carousel index based on scroll progress
  useEffect(() => {
    const unsubscribe = scrollYProgress.on('change', (value) => {
      setDebugProgress(Math.round(value * 100))
      ...
      } else if (value >= 0.10 && value < 0.22) {
        setIsExploding(true)
        setShowCarousel(false)
      } else if (value >= 0.22 && value < 0.75) {
        setIsExploding(false)
  ...
  return (
    <>
      ...
      */}
      <div 
        ref={containerRef}
        className="relative bg-gradient-to-b from-[#FFFBF5] via-amber-50/20 to-[#FFFBF5]"
        style={{ height: '250vh' }}
      >
        {/* Sticky Wrapper */}
        <div 
        ...
        >
          ...
          <div 
            ...
            `}
          >
            <div className="text-5xl md:text-7xl lg:text-8xl tracking-tight text-center">
            ...
            <motion.p
              className="mt-8 text-xl text-stone-500 text-center max-w-md"
              initial={{ opacity: 0, y: 20 }}
              animate={{ 
