...
export function Select({
  ...
  return (
    <div className="w-full" ref={containerRef}>
      {label && (
        <label className="block text-base lg:text-sm font-medium text-slate-700 mb-1.5">
          {label}
        </label>
      )}
          ...
          <div className="absolute z-50 w-full mt-1 bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
            {searchable && (
              <div className="p-2 border-b border-slate-100">
                <input
                  ...
                  onKeyDown={handleKeyDown}
                  placeholder="Search..."
                  className="w-full h-8 px-3 rounded-lg border border-slate-200 text-base lg:text-sm
            ...
            <div className="max-h-60 overflow-y-auto py-1">
              {filteredOptions.length === 0 ? (
                <div className="px-3 py-6 text-base lg:text-sm text-slate-400 text-center">
                  No options found
                </div>
              ) : (
