...
export function FairwayTripCard({ itinerary, selected, now, onSelect }: FairwayTripCardProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'group relative flex w-full items-start gap-3 rounded-card p-4 text-left',
        ...
        selected
          ? 'border border-accent-300 bg-accent-50/60 shadow-soft'
          : 'border border-border-subtle bg-surface shadow-flat hover:-translate-y-px hover:border-border-strong hover:shadow-raise',
      )}
    >
      <span
        className={cn(
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 flex items-center gap-1.5 font-fw-sans text-caption text-text-tertiary">
          <MapPin aria-hidden className="h-3.5 w-3.5 shrink-0" />
          <span className="truncate">{itinerary.destination || 'Destination TBD'}</span>
        </p>
        <p className="mt-0.5 flex items-center gap-1.5 font-fw-sans text-caption text-text-tertiary tabular-nums">
