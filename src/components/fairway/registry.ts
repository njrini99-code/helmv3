...
export const FAIRWAY_REGISTRY: readonly FairwayRegistryEntry[] = [
  ...
  { name: 'AgendaStrip', category: 'data-viz', status: 'new', archetypes: ['A'], material: 'matte', bestFor: ['a compact same-day hour axis for a day\'s events'], avoidFor: ['multi-day ranges', 'more than ~8 events (use Filmstrip)'], replaces: ['a plain seam list as the only reading of "today"'], mobile: '3 hour labels', desktop: '6 hour labels', examples: ['Coach home · Today'] },
  ...
  { name: 'ScoringHistogram', category: 'data-viz', status: 'new', archetypes: ['C', 'E'], material: 'green', bestFor: ['a single round\'s scoring mix as five fixed buckets (eagle through double-plus)', 'a hero panel restricted to one round'], avoidFor: ['a season-long distribution (use TrendChart)', 'a light/cream surface (bars are hardcoded for the accent-800/900 green panel)'], replaces: ['a bare "Mix: 3 pars, 6 bogeys" text line'], mobile: '', desktop: '', examples: ['Round review hero'] },
  { name: 'DrivingDotStrip', category: 'data-viz', status: 'new', archetypes: ['C', 'E'], material: 'matte', bestFor: ['fairway hit/miss by hole across a full round, including par-3 no-target holes'], avoidFor: ['fewer than a full round of holes', 'shot-level miss dispersion (use ShotDispersion)'], replaces: ['a driving accuracy percentage with no per-hole detail'], mobile: 'compresses width per dot, never scrolls', desktop: '', examples: ['Round review breakdown'] },
  { name: 'ScoreField', category: 'data-viz', status: 'new', archetypes: ['A', 'B', 'E'], material: 'matte', bestFor: ['every player\'s rounds on one shared date axis (bars rise over par, drop under)', 'the home and roster stage instrument'], avoidFor: ['a single player (use Ribbon)', 'fewer than two rounds per row'], replaces: ['a ranked avg table', 'per-player sparklines with no shared axis'], mobile: 'identity row above the strip, bars stay 44px tall', desktop: 'one row per player, avg and trend at the row end', examples: ['Coach home'] },
