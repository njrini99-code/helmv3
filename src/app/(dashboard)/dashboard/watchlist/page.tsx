...
import { IconStar, IconStarFilled, IconTrash, IconUser, IconSearch } from '@/components/icons';
...
export default function WatchlistPage() {
  ...
  async function handlePriorityToggle(watchlistId: string, currentPriority: number | null) {
    try {
      const newPriority = currentPriority === 1 ? 0 : 1;
      await updateWatchlistPriority(watchlistId, newPriority === 1);
      // Update local state
      setWatchlist(prev => prev.map(item =>
        item.id === watchlistId
