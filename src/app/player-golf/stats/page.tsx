...
import { calculateStatsFromShots, formatStat, formatStatInt, type RawShot, type HoleInfo, type RoundInfo, type GolfStats } from '@/lib/utils/golf-stats-calculator-shots';
import { IconChartBar, IconGolf, IconTarget, IconFlag, IconTrendingUp, IconAward } from '@/components/icons';
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Scoring Average" value={formatStat(stats.scoringAverage, '', 2)} highlight large />
        <StatCard label="Best Round" value={formatStatInt(stats.bestRound)} />
        <StatCard label="Worst Round" value={formatStatInt(stats.worstRound)} />
        <StatCard label="Rounds Played" value={formatStatInt(stats.roundsPlayed)} />
      </div>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard 
          ...
          subValue="yards avg"
        />
        <StatCard label="Fairway %" value={formatStat(stats.fairwayPercentage, '%')} />
        <StatCard 
          label="Fairways/Round" 
          value={stats.fairwayOpportunities > 0 && stats.roundsPlayed > 0 
            ? formatStat(stats.fairwaysHit / stats.roundsPlayed, '', 1) 
            : '-'} 
        />
      </div>
...
function PuttingStats({ stats }: { stats: GolfStats }) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Putts / Round" value={formatStat(stats.puttsPerRound, '', 1)} highlight large />
        <StatCard label="Putts / GIR" value={formatStat(stats.puttsPerGir, '', 2)} />
        <StatCard label="3-Putts / Round" value={formatStat(stats.threePuttsPerRound, '', 2)} />
        <StatCard label="1-Putts Total" value={formatStatInt(stats.onePuttsTotal)} />
      </div>
...
function ScramblingStats({ stats }: { stats: GolfStats }) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Scrambling %" value={formatStat(stats.scramblingPercentage, '%')} highlight large />
        <StatCard label="Sand Save %" value={formatStat(stats.sandSavePercentage, '%')} />
        <StatCard label="Scrambles Made" value={`${stats.scramblesMade}/${stats.scrambleAttempts}`} />
        <StatCard label="Penalties / Round" value={formatStat(stats.penaltiesPerRound, '', 2)} />
      </div>
...
export default function PlayerStatsPage() {
  ...
  const [roundsWithoutShots, setRoundsWithoutShots] = useState<string[]>([]);
  ...
  async function loadPlayerStats() {
    ...
    if (!player) {
      // Not a player - redirect
      window.location.href = '/player-golf';
      return;
    }
    ...
    if (!rounds || rounds.length === 0) {
      setStats(calculateStatsFromShots([], [], []));
      setLoading(false);
      return;
    }
    ...
    // Fetch shots for all rounds (SOURCE OF TRUTH)
    const { data: shots } = await supabase
      .from('golf_shots')
      .select('*')
      .in('round_id', roundIds);

    // Set shot count for display
    setShotCount(shots?.length || 0);

    // Validate shots and log warnings
    if (shots && shots.length > 0) {
      shots.forEach((shot, idx) => {
        const missing: string[] = [];
        if (!shot.shot_type) missing.push('shot_type');
        if (!shot.result) missing.push('result');
        if (shot.shot_number === null || shot.shot_number === undefined || shot.shot_number < 1) {
          missing.push('shot_number (invalid)');
        }

        if (missing.length > 0) {
          console.warn(`Shot validation warning [Shot ${idx + 1}, Round ${shot.round_id}]: Missing or invalid fields: ${missing.join(', ')}`);
        }
      });
    }

    // Check for rounds without shots
    const roundsWithNoShots: string[] = [];
    if (rounds && shots) {
      rounds.forEach(round => {
        const roundShots = shots.filter(s => s.round_id === round.id);
        if (roundShots.length === 0) {
          roundsWithNoShots.push(`${round.course_name} (${new Date(round.round_date).toLocaleDateString()})`);
        }
      });
    }
    setRoundsWithoutShots(roundsWithNoShots);

    // Calculate stats from raw shot data with error handling
    try {
      const calculatedStats = calculateStatsFromShots(
        (shots || []) as RawShot[],
        (holes || []) as HoleInfo[],
        (rounds || []) as RoundInfo[]
      );
      ...
      console.error('Error calculating stats from shots:', error);
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            {stats?.roundsPlayed || 0} rounds • {stats?.holesPlayed || 0} holes
          </p>
          <div className="flex items-center gap-2 text-xs text-green-600 mt-2">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
            </svg>
            <span>Calculated from {shotCount} shot records</span>
          </div>
        </div>

        {/* Warning Banner for Rounds Without Shots */}
        {roundsWithoutShots.length > 0 && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-800 mb-2">
                  The following rounds have no shot records. Stats may be incomplete:
                </p>
                <ul className="text-xs text-amber-700 space-y-0.5">
                  {roundsWithoutShots.map((round, idx) => (
                    <li key={idx}>• {round}</li>
                  ))}
                </ul>
              </div>
