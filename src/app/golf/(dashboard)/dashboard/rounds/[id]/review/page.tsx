...
export default function RoundReviewPage() {
  ...
  const generateReview = useCallback(async (options?: { ifMissing?: boolean }) => {
    if (!round) return;
    ...
    try {
      const result = await generateAndStoreRoundReview(roundId, round.player_id, options);
  ...
  // Auto-generate the FIRST review once, after mount, for a viewer who opened
  // this round (never during render). Audit DATA-05: the guard was state, so
  // React StrictMode's double effect (and any re-run inside one commit) saw it
  // still false and fired a second generate. A ref keyed on the round is
  // synchronous and never reset in cleanup. The server side already refuses to
  // duplicate work: `generateAndStoreRoundReview` single-flights per round and
  // rate-limits any regenerate once a row exists. Whether a first review should
  // wait for an explicit "Generate" tap was settled by the owner (2026-09-24):
  // keep auto-generate, with `ifMissing` so the server returns an existing
  // review instead of computing a second one.
  const autoGeneratedForRef = useRef<string | null>(null);
  useEffect(() => {
    if (loadingRound || loadingStoredReview || !round || storedReview || generatingReview) return;
    if (autoGeneratedForRef.current === roundId) return;
    autoGeneratedForRef.current = roundId;
    void generateReview({ ifMissing: true });
  }, [loadingRound, loadingStoredReview, round, storedReview, generatingReview, generateReview, roundId]);
  ...
  // Raw `text-red-500` + `bg-accent-fill` are gone.
  if (error) {
    return (
      <div className={fairwayScope('min-h-full bg-canvas')}>
