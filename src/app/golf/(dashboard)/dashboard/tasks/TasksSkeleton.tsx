...
/**
 * Route-local, Fairway-shaped skeleton for the Tasks board — shape-matches
 * FairwayTasks (max-w-[1280px] ViewHeader masthead → filter pills → search →
 * a `grid lg:grid-cols-3` task list + Templates rail), mirroring the same
 * markup already authored in this route's `loading.tsx` Suspense fallback so
 * the client component's own `if (loading)` branch paints the identical
 * shape instead of the legacy `max-w-[720px]` / `surface-matte` skeleton.
 */
