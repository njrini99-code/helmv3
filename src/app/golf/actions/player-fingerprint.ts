...
export async function getPlayerFingerprint(
  ...
  const [
    ...
    supabase
      .from('golf_rounds')
      // `golf_rounds` stores `score_to_par` directly; there is no `course_par`
      // column (selecting it 400s the whole PostgREST request). All par deltas
      // below derive from `score_to_par` (== total_score − par).
      .select(
