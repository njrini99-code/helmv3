...
function buildPressureSection(
  ...
  // `golf_rounds.score_to_par` already encodes (total_score − course par) for
  // the round, so we use it directly rather than re-deriving from a course-par
  // column. (The table has no `course_par`; selecting it threw
  // "column golf_rounds.course_par does not exist" and dropped the rounds.)
