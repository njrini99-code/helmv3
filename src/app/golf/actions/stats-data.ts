...
async function queryDetailedStatsWithClient(
  ...
  try {
    ...
    // 57014) or failed, so we fall back to round-level stats — the user still
    // gets data. Record it for perf tracking, but it is NOT an unhandled error,
    // so keep it out of Sentry (issue 4K). Perf follow-up tracked separately.
