...
import { getDemoSessions } from '@/app/golf/actions/demo-tracking';
...
function shortUA(ua: string | null): string {
  if (!ua) return '—';
  // Grab the first parenthesised segment + product token at the start
...
export default async function DemoSessionsPage() {
  let result: Awaited<ReturnType<typeof getDemoSessions>> | null = null;
  let fetchError: string | null = null;

  try {
    result = await getDemoSessions();
  } catch (err) {
    fetchError = err instanceof Error ? err.message : 'Failed to load demo sessions.';
  }
                ...
                {sessions.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-warm-600 whitespace-nowrap">
                      {s.school ?? <span className="text-warm-300">—</span>}
                    </td>
