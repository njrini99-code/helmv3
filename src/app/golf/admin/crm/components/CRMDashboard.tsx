...
export function CRMDashboard({
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-[1400px] mx-auto">
      ...
      {allNewLeads && (
        <div className="rounded-card border border-border-subtle bg-surface [box-shadow:var(--fw-shadow-card)] p-8 text-center">
          <div className="w-16 h-16 rounded-card bg-accent-50 flex items-center justify-center mx-auto mb-5">
            <IconRocket size={28} className="text-accent-700" />
          </div>
          <h3 className="text-xl font-bold text-text-primary mb-2">Ready to start your pipeline</h3>
          <p className="text-sm text-text-tertiary max-w-lg mx-auto mb-6 leading-relaxed">
            You have <span className="font-semibold text-text-secondary">{stats.total} coaches</span> ready to work.
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-card border border-border-subtle bg-surface [box-shadow:var(--fw-shadow-card)] p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-fw-sm bg-accent-50 flex items-center justify-center">
                <IconChartBar size={16} className="text-accent-700" />
              </div>
              <div>
                <h3 className="text-sm font-semibold text-text-primary">Pipeline Funnel</h3>
                <p className="text-xs text-text-tertiary">Conversion by stage</p>
              </div>
            </div>
            <Button variant="ghost"
              onClick={() => onNavigate('list', 'board')}
              ...
              return (
                <div key={stage.id} className="flex items-center gap-2 sm:gap-4 group">
                  <div className="w-20 sm:w-28 flex items-center gap-2 flex-shrink-0 justify-end">
                    ...
                    <p className="text-xs sm:text-sm font-medium text-text-secondary group-hover:text-text-primary transition-colors truncate">{stage.label}</p>
                  </div>
                  <div className="flex-1 relative">
                    <div className="h-8 rounded-fw-sm bg-surface-sunken/80 overflow-hidden">
                      <div
                        className={cn(
                          'h-full rounded-fw-sm transition-[width,opacity] duration-700 ease-out flex items-center',
                          stage.gradient,
                      ...
                      >
                        {widthPct > 15 && (
                          <span className="text-text-on-accent/90 text-xs font-semibold pl-3">{stage.pct}%</span>
                        )}
                      </div>
                    </div>
        ...
        <div className="rounded-card border border-border-subtle bg-surface [box-shadow:var(--fw-shadow-card)] p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-fw-sm bg-surface-tint flex items-center justify-center">
                <IconTrophy size={16} className="text-text-secondary" />
              </div>
              <div>
                <h3 className="text-sm font-semibold text-text-primary">Top Conferences</h3>
                <p className="text-xs text-text-tertiary">{conferenceStats.length} shown</p>
              </div>
            </div>
          </div>
                ...
                <div key={conf.name} className="flex items-center gap-3 group">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-bold text-text-primary tabular-nums ml-2">{conf.count}</span>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="rounded-card border border-border-subtle bg-surface [box-shadow:var(--fw-shadow-card)] p-5 flex flex-col">
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-8 h-8 rounded-fw-sm bg-fw-warning-bg flex items-center justify-center">
              <IconClock size={16} className="text-fw-warning-ink" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-text-primary">Needs attention</h3>
              <p className="text-xs text-text-tertiary">Act on these from Today</p>
            </div>
          </div>
          <div className="space-y-3 flex-1">
            <div className="flex items-center justify-between">
              <span className="text-sm text-text-secondary">Follow-ups due</span>
              <span className="text-sm font-semibold text-fw-warning-ink tabular-nums">{loading ? '—' : followUpsDueToday.length}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-text-secondary">Stale leads (7d+)</span>
          ...
          <Button
            ...
            className="mt-4 w-full justify-center rounded-fw-md bg-canvas hover:bg-surface-tint text-sm font-medium text-text-primary min-h-[40px]"
          >
            Open Today queue
          </Button>
        </div>
        ...
        {/* Recent Activity */}
        <div className="rounded-card border border-border-subtle bg-surface [box-shadow:var(--fw-shadow-card)] p-5 lg:col-span-2">
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-8 h-8 rounded-fw-sm bg-accent-50 flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-xs text-text-tertiary">Latest pipeline changes</p>
            </div>
          </div>
          {loading ? (
            <ListSkeleton items={3} />
          ) : recentlyUpdated.length === 0 ? (
            <EmptyState icon={<IconZap size={18} className="text-text-tertiary" />} title="No activity yet" subtitle="Start working leads to see activity" />
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="rounded-card border border-border-subtle bg-surface [box-shadow:var(--fw-shadow-card)] p-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-fw-md bg-accent-50 flex items-center justify-center flex-shrink-0">
                  <IconZap size={18} className="text-accent-700" />
                </div>
                <div>
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-8 h-8 rounded-fw-sm bg-surface-sunken flex items-center justify-center">
              <IconUsers size={16} className="text-text-secondary" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-text-primary">Division Breakdown</h3>
              <p className="text-xs text-text-tertiary">By division</p>
            </div>
          </div>
...
function KPICard({
  ...
  return (
    <div className={cn(
      ...
      'rounded-fw-md border border-border-subtle bg-surface [box-shadow:var(--fw-shadow-card)] sm:rounded-card',
      'shadow-flat',
      'p-3.5 sm:p-5',
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-caption sm:text-xs text-text-tertiary uppercase tracking-wider font-semibold">{label}</p>
          <p className="text-xl sm:text-2xl font-bold text-text-primary tabular-nums tracking-tight mt-1">{value}</p>
          <p className="hidden sm:block text-xs text-text-tertiary mt-1">{detail}</p>
        </div>
        <div className={cn(
          'w-8 h-8 rounded-fw-sm flex items-center justify-center flex-shrink-0',
  ...
  return (
    <div className="flex items-center gap-4 p-4 rounded-fw-md border border-border-subtle bg-surface-tint/60">
      ...
      <div>
        ...
        <p className="text-xs text-text-tertiary">{pct}% of total</p>
      </div>
    </div>
  );
}
...
function CoachRow({
  ...
  return (
    <Button variant="ghost"
      ...
      className="w-full flex items-center gap-3 p-2 rounded-fw-md hover:bg-surface-sunken/60 transition-colors text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-border-focus/50"
...
// Thin local alias over the Fairway primitive so this file's existing
// `icon/title/subtitle` call sites keep working while the rendering (chip,
// type roles, reveal motion, role="status") comes from the design system.
function EmptyState({ icon, title, subtitle }: { icon: React.ReactNode; title: string; subtitle: string }) {
  return <FairwayEmptyState variant="subtle" icon={icon as React.ReactElement} title={title} description={subtitle} />;
}
