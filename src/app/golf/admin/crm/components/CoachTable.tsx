...
export function CoachTable({
  ...
  const getRowBg = (coach: Coach) => {
    if (coach.is_starred) return 'bg-amber-50/50';
    if (coach.priority === 2) return 'bg-orange-50/50';
    if (coach.status === 'customer') return 'bg-emerald-50/30';
    return 'bg-white';
  };
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gradient-to-r from-emerald-50/50 to-teal-50/50 border-b border-white/30">
              ...
              <th 
                className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-700"
              ...
              <th 
                className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-700 w-24"
              ...
              <th 
                className="text-left px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-700 w-28"
                ...
                >
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      {coach.priority > 0 && (
                        <span className="text-base">{priorityConfig[coach.priority]?.icon}</span>
                      )}
                  ...
                  {/* School */}
                  <td className="px-4 py-2.5">
                    <div className="font-medium text-slate-800 truncate max-w-[200px]">{coach.school}</div>
                    ...
                    <div className="relative">
                      <button
                        ...
                        className={cn(
                          ...
                          statusConfig[coach.status]?.ringColor
                        )}
                      >
                      ...
                      {openDropdown?.id === coach.id && openDropdown.type === 'status' && (
                        <div className="absolute z-50 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl py-2 min-w-[160px]">
                          {ALL_STATUSES.map((status) => (
                            <button
                              ...
                              className={cn(
                                ...
                                coach.status === status && 'bg-slate-100'
                              )}
                            >
                  ...
                  <td className="px-4 py-2.5" onClick={(e) => e.stopPropagation()}>
                    <div className="relative">
                      ...
                      {openDropdown?.id === coach.id && openDropdown.type === 'priority' && (
                        <div className="absolute z-50 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl py-2 min-w-[120px]">
                          {[2, 1, 0].map((p) => (
                            <button
                              key={p}
                              onClick={() => {
      ...
      <div className="bg-gradient-to-r from-emerald-50/30 to-teal-50/30 border-t border-white/30 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="text-sm border border-slate-200 rounded-lg px-2 py-1.5 bg-white focus:ring-2 focus:ring-emerald-500"
          >
            {PAGE_SIZES.map(size => (
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => setPage(1)}
              disabled={page === 1}
              className="px-2 py-1 rounded-lg hover:bg-white disabled:opacity-40 text-sm font-medium"
            >
              ««
            </button>
            <button
