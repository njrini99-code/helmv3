...
describe('AnalyticsPage — player_type gate', () => {
  ...
  it('is closed for EVERY player type while the recruiting module is sunset', async () => {
    for (const player_type of ['college', 'high_school', 'juco', 'showcase']) {
      ...
      await expect(AnalyticsPage(), player_type).rejects.toThrow('REDIRECT:/baseball/player/today');
    }
    expect(analyticsClientMock).not.toHaveBeenCalled();
  });

  it('renders an honest "not available" state for a college player, WITHOUT rendering AnalyticsClient — restoration path', async () => {
    getSessionProfileMock.mockResolvedValue({
      userId: 'u1',
      role: 'player',
      coach: null,
    ...
    const element = await withRecruitingEnabled(() => AnalyticsPage());
    render(element);
  ...
  it('renders the real AnalyticsClient for a juco player — restoration path', async () => {
    getSessionProfileMock.mockResolvedValue({
      userId: 'u1',
      role: 'player',
      coach: null,
