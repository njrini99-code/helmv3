            ...
            return (
              <div key={msg.id} className={cn('flex', isOwn ? 'justify-end' : 'justify-start')}>
                <div className={cn('max-w-md', isOwn ? 'order-2' : 'order-1')}>
                  ...
                  <div className={cn('px-4 py-2 rounded-2xl', isOwn ? 'bg-brand-600 text-white rounded-br-md' : 'bg-cream-200 text-warm-900 rounded-bl-md')}>
                    <p className="text-sm">{msg.content}</p>
                  </div>
                  <p className={cn('text-xs text-warm-400 mt-1', isOwn ? 'text-right' : 'text-left')}>
