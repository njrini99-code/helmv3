// =============================================================================
// SeasonSettingsPage — BaseballUnauthorizedError must redirect, not raw-throw.
//
// listSeasons (withBaseballAction) independently re-resolves auth, so a
...
vi.mock('@/components/baseball/settings/SeasonSettingsClient', () => ({
  SeasonSettingsClient: () => null,
}));

import SeasonSettingsPage from '../page';

describe('SeasonSettingsPage — BaseballUnauthorizedError redirects instead of raw-throwing', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getUser.mockResolvedValue({ data: { user: { id: 'coach-1' } } });
  });

  it('redirects to /baseball/login (with returnTo) when the session expired between the auth check and the seasons fetch', async () => {
    mocks.listSeasons.mockRejectedValue(new FakeBaseballUnauthorizedError());

    await expect(SeasonSettingsPage()).rejects.toThrow(
      'REDIRECT:/baseball/login?returnTo=' +
        encodeURIComponent('/baseball/dashboard/settings/season'),
    );
  });

  it('re-throws any OTHER error (a real failure for a signed-in, authorized coach) instead of redirecting', async () => {
    mocks.listSeasons.mockRejectedValue(new Error('seasons query failed'));

    await expect(SeasonSettingsPage()).rejects.toThrow('seasons query failed');
    expect(mocks.redirect).not.toHaveBeenCalled();
  });

  it('renders normally when the seasons fetch succeeds', async () => {
    mocks.listSeasons.mockResolvedValue({ seasons: [] });

    const element = await SeasonSettingsPage();
