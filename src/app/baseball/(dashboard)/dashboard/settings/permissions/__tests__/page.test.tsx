// =============================================================================
// PermissionsPage — BaseballUnauthorizedError must redirect, not raw-throw.
//
// getPermissionMatrix (withBaseballAction) independently re-resolves auth, so
// a session that expires between the page's own supabase.auth.getUser()
...
describe('PermissionsPage — BaseballUnauthorizedError redirects instead of raw-throwing', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getUser.mockResolvedValue({ data: { user: { id: 'coach-1' } } });
  });

  it('redirects to /baseball/login (with returnTo) when the session expired between the auth check and the matrix fetch', async () => {
    mocks.getPermissionMatrix.mockRejectedValue(new FakeBaseballUnauthorizedError());

    await expect(PermissionsPage()).rejects.toThrow(
      'REDIRECT:/baseball/login?returnTo=' +
        encodeURIComponent('/baseball/dashboard/settings/permissions'),
    );
  });

  it('re-throws any OTHER error (a real failure for a signed-in, authorized coach) instead of redirecting', async () => {
    mocks.getPermissionMatrix.mockRejectedValue(new Error('permission matrix query failed'));

    await expect(PermissionsPage()).rejects.toThrow('permission matrix query failed');
    expect(mocks.redirect).not.toHaveBeenCalled();
  });

  it('renders normally when the matrix fetch succeeds', async () => {
    mocks.getPermissionMatrix.mockResolvedValue({ groups: [], viewerCanInviteStaff: false });

    const element = await PermissionsPage();
