// =============================================================================
// ProgramSettingsPage — BaseballUnauthorizedError must redirect, not raw-throw.
//
// getProgramSettings (withBaseballAction) independently re-resolves auth, so
...
vi.mock('@/components/baseball/settings/ProgramSettingsClient', () => ({
  ProgramSettingsClient: () => null,
}));

import ProgramSettingsPage from '../page';

describe('ProgramSettingsPage — BaseballUnauthorizedError redirects instead of raw-throwing', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('redirects to /baseball/login (with returnTo) when the session expired between the auth check and the settings fetch', async () => {
    ...
    await expect(ProgramSettingsPage()).rejects.toThrow(
  ...
  it('re-throws any OTHER error (a real failure for a signed-in, authorized coach) instead of redirecting', async () => {
    mocks.getProgramSettings.mockRejectedValue(new Error('program settings query failed'));

    await expect(ProgramSettingsPage()).rejects.toThrow('program settings query failed');
  ...
  it('renders normally when the settings fetch succeeds', async () => {
    ...
    const element = await ProgramSettingsPage();
