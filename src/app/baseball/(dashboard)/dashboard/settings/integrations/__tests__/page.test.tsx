// =============================================================================
// IntegrationsPage — BaseballUnauthorizedError must redirect, not raw-throw.
//
// getProgramSettings and listIntegrations (both withBaseballAction)
...
describe('IntegrationsPage — BaseballUnauthorizedError redirects instead of raw-throwing', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('redirects to /baseball/login (with returnTo) when the session expired before the FIRST getter call', async () => {
    ...
    await expect(IntegrationsPage()).rejects.toThrow(
      'REDIRECT:/baseball/login?returnTo=' +
        encodeURIComponent('/baseball/dashboard/settings/integrations'),
    );
  ...
  it('redirects to /baseball/login (with returnTo) when the session expired before the SECOND getter call', async () => {
    mocks.getProgramSettings.mockResolvedValue({
      teamName: 'Rini U',
      viewerCanManageSettings: true,
    });
    mocks.listIntegrations.mockRejectedValue(new FakeBaseballUnauthorizedError());
  ...
  it('re-throws any OTHER error (a real failure for a signed-in, authorized coach) instead of redirecting', async () => {
    ...
    mocks.listIntegrations.mockRejectedValue(new Error('integrations query failed'));

    await expect(IntegrationsPage()).rejects.toThrow('integrations query failed');
