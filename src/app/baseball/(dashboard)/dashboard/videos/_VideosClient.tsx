...
export default function VideosClient() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      // Server action: capability/player-access enforced, storage cleanup +
      // has_video flag handled server-side, ownership re-checked.
      const res = await deleteMyVideo({ videoId: deleteConfirm.id });
      if (!res.success) {
  ...
  const setPrimary = async (videoId: string) => {
    ...
    try {
      ...
      if (!res.success) {
        showToast(res.error || 'Failed to update primary video', 'error');
        return;
      }
      showToast('Primary video updated', 'success');
      fetchVideos();
    } catch {
      showToast('Failed to update primary video', 'error');
    }
  };
  ...
  const handleViewVideo = async (video: VideoWithPlayer) => {
    ...
    // Increment view count via the server action (server reads the current
    // tally rather than trusting a client-supplied count). Fire and forget.
  ...
  if (authLoading) return <><Header title="Videos" /><div className="p-4 sm:p-6 lg:p-8"><SkeletonVideos /></div></>;
  if (loading) return <><Header title="Videos" /><div className="p-4 sm:p-6 lg:p-8"><SkeletonVideos /></div></>;

  function applyFilters(video: VideoWithPlayer): boolean {
    ...
    const matchesType = !filterType || video.video_type === filterType;
    return !!matchesSearch && matchesType;
  }

  const filteredMyVideos = myVideos.filter(applyFilters);
  const filteredTeamVideos = teamVideos.filter(applyFilters);
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        {/* Search + Filter Bar (show when there are videos) */}
        {!hasNoVideos && (
            ...
            <CardContent className="p-3 sm:p-4">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                ...
                >
                  ...
                  {filterType ? 'Filtered' : 'Filters'}
                </Button>
              </div>
              {showFilterPanel && (
                <div className="mt-3 pt-3 border-t border-warm-100 flex flex-wrap gap-2">
                  <span className="text-xs font-semibold text-warm-500 self-center mr-1">Type:</span>
                  <button
                    type="button"
                    onClick={() => setFilterType('')}
                    className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-colors ${
                      !filterType ? 'bg-primary-100 text-primary-700 border-primary-200' : 'bg-white text-warm-600 border-warm-200 hover:border-warm-300'
                    }`}
                  >
                    All
                  </button>
                  {videoTypes.map(vt => (
                    <button
                      key={vt.value}
                      type="button"
                      onClick={() => setFilterType(vt.value)}
                      className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-colors ${
                        filterType === vt.value ? 'bg-primary-100 text-primary-700 border-primary-200' : 'bg-white text-warm-600 border-warm-200 hover:border-warm-300'
                      }`}
                    >
                      {vt.label}
                    </button>
