...
export default function HSCoachDashboardPage() {
    ...
    const { data: rosterData } = await supabase
      ...
      .select(`
        ...
        player:baseball_players!inner(
          ...
          primary_position, grad_year, gpa, recruiting_activated
        )
      `)
      .eq('team_id', currentTeamId);

    if (rosterData) {
      const typedRoster = rosterData as TeamMember[];
  ...
  return (
    <>
      <Header title="Team Dashboard" subtitle="High School Team Overview" />
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            change={
              avgGPA && avgGPA >= 3.0 ? 'Excellent standing'
              : avgGPA && avgGPA >= 2.5 ? 'Good standing'
              : avgGPA ? 'Needs attention' : undefined
            }
            changeType={avgGPA && avgGPA >= 3.0 ? 'positive' : avgGPA && avgGPA >= 2.5 ? 'neutral' : 'negative'}
          />
          <StatCard label="Active Dev Plans" value={devPlanCount} icon={IconNote} />
          <StatCard
