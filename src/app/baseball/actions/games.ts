...
export async function deleteGame(gameId: string): Promise<{ success: boolean; error?: string }> {
  ...
  const { error } = await supabase.from('baseball_games').delete().eq('id', gameId);
...
export async function saveBoxScoreBatting(
  ...
  await supabase.from('baseball_box_score_batting').delete().eq('game_id', gameId);
  ...
  const { error } = await supabase.from('baseball_box_score_batting').insert(rows);
...
export async function saveBoxScorePitching(
  ...
  await supabase.from('baseball_box_score_pitching').delete().eq('game_id', gameId);
  ...
  const { error } = await supabase.from('baseball_box_score_pitching').insert(rows);
...
export async function markGameCompleted(
  ...
  const { data: battingPlayers } = await supabase
    .from('baseball_box_score_batting')
    .select('player_id')
    .eq('game_id', gameId);

  const { data: pitchingPlayers } = await supabase
    .from('baseball_box_score_pitching')
    .select('player_id')
    .eq('game_id', gameId);
  ...
  await Promise.all(
    allPlayerIds.map((playerId) =>
      supabase.rpc('recalculate_baseball_season_stats', {
...
export async function recalculateAllSeasonStats(
  ...
  const { error } = await supabase.rpc('recalculate_team_baseball_season_stats', {
    p_team_id: teamId,
    p_season_year: year,
  });
