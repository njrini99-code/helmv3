 ...
 *
 * Reads from `baseball_demo_sessions` via the service-role (admin) client
 * because that table is RLS-locked (no public select policy). The
 * admin-identity check runs FIRST via the user-scoped client so we never
 * expose the admin bypass to non-admins. Mirrors
...
export interface BaseballDemoSession {
  id: string;
  name: string;
  email: string;
  program: string | null;
...
export interface GetBaseballDemoSessionsResult {
  sessions: BaseballDemoSession[];
  total: number;
}
...
async function getBaseballDemoSessionsImpl(): Promise<GetBaseballDemoSessionsResult> {
  ...
  //    NOTE: `baseball_demo_sessions` is not yet in the generated Database
  //    type until `npm run db:types` is re-run against the new migration. We
  //    cast through `unknown` here; once regenerated the cast becomes
  ...
  return {
    sessions: (data ?? []) as BaseballDemoSession[],
    total: count ?? 0,
  };
}
