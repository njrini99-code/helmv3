...
//   3. Lead-row capture into baseball_demo_sessions alongside sign-in.
...
describe('enterBaseballDemo — lead-row capture', () => {
  it('inserts a baseball_demo_sessions row with visitor, quality, and request metadata', async () => {
    await expect(enterBaseballDemo(VALID_INPUT)).rejects.toThrow('REDIRECT:/baseball/dashboard?demo=1');
