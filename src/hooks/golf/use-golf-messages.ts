...
/** Cache key for one thread's last fetched page. */
export function messagesCacheKey(conversationId: string): string {
  return `golf.messages:${conversationId}`;
}

/** Cache key for one viewer's conversation rail. */
export function conversationsCacheKey(userId: string): string {
  return `golf.conversations:${userId}`;
}
...
export function useGolfMessages(conversationId: string, viewerUserId?: string | null) {
  ...
  const warm = conversationId
    ? readCachedResource<MessageWithReadStatus[]>(messagesCacheKey(conversationId))
    : null;
  ...
  const fetchMessages = useCallback(async () => {
    ...
    if (!readCachedResource<MessageWithReadStatus[]>(messagesCacheKey(conversationId))) {
      setLoading(true);
    }
    setError(false);
    ...
    writeCachedResource(messagesCacheKey(conversationId), fresh);
    setMessages(fresh);
    setLoading(false);
  ...
  useEffect(() => {
    ...
    writeCachedResource(messagesCacheKey(conversationId), cacheableMessages(messages));
  ...
  useEffect(() => {
    ...
    const cached = readCachedResource<MessageWithReadStatus[]>(messagesCacheKey(conversationId));
    if (cached) {
 ...
 */
export function useGolfConversations(viewerUserId?: string | null) {
  const warm = viewerUserId
    ? readCachedResource<GolfConversationWithMeta[]>(conversationsCacheKey(viewerUserId))
    : null;
  ...
  const fetchConversations = useCallback(async () => {
    ...
    if (!readCachedResource<GolfConversationWithMeta[]>(conversationsCacheKey(userId))) {
      setLoading(true);
    }
    setError(false);
    ...
    writeCachedResource(conversationsCacheKey(userId), transformedConversations);
