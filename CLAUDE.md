...
- **2026-05-27 — Inngest v4 API shape against installed v4 (PR #102 fix).** TS portability error masked the call-shape mismatch; 9+ hours of broken prod. *Caught by SDK version-drift rule #11 + hotfix PR #106 (admin-merged) aligning to v4.*
- **2026-05-27 — Dashboard-era schema debt on `golf_shots` (`shot_type`, `round_id`, `hole_number` undeclared).** Migration 040 referenced columns added to prod via the Supabase dashboard; fresh DB diverged. *Caught by Supabase lint + RLS CI; fixed defensively in migration 040 with `IF NOT EXISTS` pattern.*
- **2026-05-27 — v3 chip/pitch reader filter against 5-value CHECK enum.** scrambling-rate dimension + hole-sequence-loader filtered `shot_type` for `'chip'`/`'pitch'` values that migration 040's CHECK constraint forbids; silent zero-row returns. Introduced in W30.5 composite-rules merge (`7bbd673b`). *Caught by three-agent v3-impact review on PR #105; fixed in commit `ac62e3c8` to accept `around_green | chip | pitch`.*
- **2026-05-27 — 67 SECURITY DEFINER functions missing `SET search_path` (rule #6 debt).** Pre-existing across shipped migrations; potential RLS bypass via search-path hijacking. *Caught by semgrep custom rule; fixed by Agent A's repair migration (`20260527160000_v3_security_definer_search_path_hardening.sql`) adding `ALTER FUNCTION ... SET search_path = 'pg_catalog, public'` per function (31 unique signatures, idempotent ALTER pattern). Semgrep rule still surfaces the historical `CREATE FUNCTION` sites because it pattern-matches DDL text; runtime behavior is fully fixed.*
- **2026-05-27 — 15 CREATE TABLE migrations shipped without same-file RLS (rule #6 debt).** Some tables relied on RLS being enabled in a later migration (true false positive on rule scope); others truly missing RLS. *Detected by semgrep custom rule; follow-up issue opened to enable RLS + canonical policies for the truly-missing subset (Agent B parallel work did not land in time).*
