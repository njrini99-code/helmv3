...
**⚠️ ALSO REQUIRED for ads:** Read the Pencil skill's ad vibe guide at `.claude/skills/pencil-golfhelm/references/ad-vibe-guide.md`. This defines the complete creative direction: floating 3D device mockups on sage canvas, editorial scattered layouts, badge annotations, and the premium sports-tech aesthetic. The vibe guide OVERRIDES older creative rules where they conflict.

**Key vibe shifts from the guide:**
- **Sage canvas** (`#B5C0AD → #8B9E82`) is now the DEFAULT background, not cream
- **Floating device mockups** with 3D depth/shadows, never flat screenshots
- **Scattered editorial layouts**, never perfectly centered compositions
- **2-4 badge/pill elements** floating around mockups as feature annotations
- **6-8 word headlines** (tighter than before), one emphasis word treatment
