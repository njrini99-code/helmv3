...
11. **NEVER invent a logo or icon** — ALWAYS use `<img src="./helm-logo.png">` for the brand mark (ship's helm wheel with golf ball). See `design-tokens.md` Brand Logo section.
12. **NEVER use emoji as icons in final creatives** — use the real SVG icons from the Icon Library in `design-tokens.md`. Emojis are placeholders only.
13. **ALWAYS render with `page.goto('file://' + path)`** — not `page.setContent()` — so local image references (logo, background photos) resolve correctly.
