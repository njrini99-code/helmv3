...
What to *verify* on a golf change. Where features live is a separate concern —
that map is `golf-feature-ownership.md`.
...
- **Coach vs. player feature ownership** — players must never see coach-only
  *controls* (Coach Intent, roster management, qualifier selection); coaches must
  ...
  ownership map: `/dashboard/roster` and `/dashboard/qualifiers` are shared
