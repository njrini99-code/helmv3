...
Functions, or a security audit, invoke `supabase` (and
`supabase-postgres-best-practices` for query/schema performance)
rather than working from memory. Install both with `npx skills add
supabase/agent-skills`; `skills-lock.json` listing them is NOT proof they
are on disk.
