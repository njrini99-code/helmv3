...
| `Playwright PR smoke (a11y)` | `pr-smoke.yml` | public-route accessibility Playwright | Advisory — gated at the `pull_request` trigger's own `paths:` filter (frontend/e2e paths) since 2026-09-06, so a non-matching PR starts zero jobs from this workflow, not a skipped one |
...
| `Capture + upload Sentry snapshots` | `sentry-snapshots.yml` | visual diff of a curated screen set against Sentry Snapshots | Advisory — `pull_request` trigger path-filtered (frontend/e2e paths) since 2026-09-06; `push` to `main` always runs to refresh the base build <!-- markdownlint-disable-line MD013 --> |
...
- **PR smoke** (`pr-smoke.yml`) — optional `Playwright PR smoke (a11y)` ~12
  min. Gated at the `pull_request` trigger's own `paths:` filter
  (frontend/e2e paths) since 2026-09-06: a non-matching PR never starts this
  workflow, so there is nothing to wait on. (The `Smoke checks` build is
  gone since 2026-09-02; `Next build` inside CI is the build verdict, ~6 min
  warm.)
