...
### Task 16a — Lock the global coverage tripwire — DONE

**Batches recap (all now confirmed landed on `feat/helm-bridge-instrumentation`, tip `880994c16` at the start of this unit):** B0-B5 (`027e3c4ce`,`24f220064`,`6af19e66d`,`a479f7d7a`,`2f5f286b7`,`20a67d2a1`), B6 (`396cc446a`), T15-CORE (`8639262de`), B7 (`eb4395f52`), B8/B9 insights remainder (`83261f54d`), B8 files (`974f43675`), B9 files (`a498625fc`) — merged via `2385bed24`/`6d4311136`/`85ed603ca`. **W15 batches (Tasks 5-14) are DONE in full.** T15b (call-site RLS-ctx threading at the 13 golf-action call sites + 2 route handlers) is a separate, still-in-flight unit — NOT touched by this commit.

- Flipped `coverage-contract.foundation.test.ts`'s global tripwire from `it.todo` to a live test. It does **not** hard-code the 76-file list — it walks `src/app/golf/actions/**` at test-run time (skipping `__tests__`), drops `crm-*.ts` + `resend-activity.ts` (spec §1.3 CRM exclusion) and the 11 "not action boundary" files (`messages.ts`, `insight-delivery-ranking.ts`, `team-category-insights-helpers.ts`, the 3 `*-types.ts` files, `recruit-documents-categories.ts`, `team-switcher.constants.ts`, `admin/rollup-a.ts`/`rollup-b.ts`/`rollup-c.shared.ts`), keeps only files containing a `'use server'` directive, then asserts every export in the discovered set + `src/app/admin/actions/triage.ts` + the 10 golf exports of `src/app/actions/messages.ts` is wrapped with a valid registry-matching `FeatureKey`. Because the file list is discovered, not enumerated, a future PR that adds an unwrapped action file to this directory fails the test automatically — no manifest maintenance required. A `>= 76` floor guards against the walk silently finding nothing.
- **Result: zero stragglers.** Ran the scanner cold before touching the test: 413 exports across the 76 discovered files + 10 golf exports of `src/app/actions/messages.ts` + 1 (`resolveTriageEvents` in `triage.ts`) = **424/424 already wrapped with valid, manifest-matching features** — B0-B9 left nothing to retrofit. Added a second assertion (`total wrapped-and-valid action count ... is exactly 424`) that re-derives this count from the same scanner + directory walk on every run, so the 424 figure is a live invariant, not a comment.
- **No exclusion-manifest widening.** The only files excluded are the pre-existing spec §1.3 manifest (CRM + non-action-boundary helpers/types/shims) — none added or broadened for this task.
- **Fixed a stale self-test fixture (found while getting this file green):** three pre-existing self-tests ("proves the harness detects gaps") asserted `alerts.ts` was still bare, but B7 wrapped it — those tests were failing at tip *before* this commit (confirmed via `git show HEAD:<path>` swap, not caused by this change). Since every real golf action file is now fully wrapped (the point of W15), there is no production file left to serve as a stable "still unwrapped" fixture. Replaced it with a new test-only, never-shipped, never-wrapped fixture (`src/lib/admin/__tests__/fixtures/unwrapped-actions.fixture.ts`, 2 exports, outside `src/app/golf/actions/**` so the global tripwire's own directory walk never sees it) and repointed the 3 self-tests at it. This is a one-line justification per the "any genuinely-required exclusion gets a one-line justification" rule: the fixture is required infrastructure for the self-tests, not a coverage exclusion.
- Gate: `coverage-contract.*` + `feature-registry.test.ts` — 52 test files / 112 tests, all green. `npx tsc --noEmit` exit 0. `npm run lint` exit 0 (2275 pre-existing baseline warnings, 0 errors, 0 new warnings in touched files). `npm test` — 3992 passed / 6 pre-existing failures, all confirmed unrelated and pre-dating this commit (files untouched by this diff, verified via `git status`): 3 in `src/lib/baseball/__tests__/program-type-nav-variants.test.ts` (baseball nav-set drift, out of scope — never touch baseball), 3 in `insight-celebration.test.ts`/`round-recap.test.ts` (the documented Next-16 `revalidatePath`-outside-request-scope invariant, same cluster flagged in every prior W15 batch entry above).
- `git diff feat/helm-bridge-command-center...HEAD --name-only | grep -iE "baseball|lifting|crm"` → 1 hit, `src/app/admin/baseball/page.tsx` — pre-existing (added in `409b169ed`, an Admin Bridge read-only "baseball hold" placeholder page under `src/app/admin/`, not `src/app/baseball/**`/`src/lib/baseball/**`), untouched by this commit. This commit's own diff touches exactly 2 files, neither baseball/lifting/crm: the foundation test file + the new fixture.
...
- **Full gate (only full-suite run of this unit):**
  ...
  - `npm run lint` → exit 0. 2275 pre-existing baseline warnings, **0 errors**, 0 new warnings.
  - `npm test` (`vitest --project unit run`, `NODE_OPTIONS=--max-old-space-size=8192`) → **3992 passed / 6 failed / 39 skipped** (4037 total across 376 test files: 372 passed, 3 failed, 1 skipped).
  - `coverage-contract.*` (all 13 batch + foundation files) + `feature-registry.test.ts`: **all green**, including the flipped foundation tripwire (`global tripwire > every non-CRM golf action export is wrapped with a valid FeatureKey` and `total wrapped-and-valid action count across the discovered area is exactly 424` — both pass, re-verified in isolation).
  - `fetch-all-rows.test.ts`: **13/13 green** in isolation, including all `rlsCtx` capture-context assertions (exactly-one-capture-on-42501, non-denial no-capture, omitted-ctx regex fallback) for both `fetchAllRows` and `fetchAllRowsResult`.
- **6 failures, all pre-existing, all excused with proof** (`git diff --stat 880994c16 HEAD -- <path>` produced **zero output** for every implicated file/dir below — i.e. these files are byte-identical between the stated base tip and this unit's merged HEAD, so whatever they do now is exactly what they did before T15b/T16a/this unit touched anything):
  1. `src/lib/baseball/__tests__/program-type-nav-variants.test.ts` (3 tests: "coach nav ORDER differs between College and Showcase", "absent programType falls back to declaration order", "re-ordering never adds or drops entries") — baseball nav-registry/nav-manifest drift (an `events`/`organization`/`teams` entry-set mismatch), unrelated to golf/admin instrumentation; `git diff --stat 880994c16 HEAD -- src/lib/baseball/` = empty (zero files changed). Out of scope per hard rule "never touch baseball/lifting/crm" — not ours to fix. Re-ran in isolation (`vitest run --project unit src/lib/baseball/__tests__/program-type-nav-variants.test.ts`) — same 3/16 failures, deterministic.
  2. `src/test/golf/actions/insight-celebration.test.ts` ("stamps metadata.celebration_shown_at on first call") and `src/app/golf/actions/__tests__/round-recap.test.ts` (2 tests: 9-hole vs 18-hole average comparisons) — the documented Next-16 `Invariant: static generation store missing in revalidatePath` error, thrown from `revalidatePath()` calls inside `insight-celebration.ts:134-136` / `round-recap.ts:109` when invoked outside a request-scope inside vitest; this is the KNOWN cluster flagged in every prior W15 batch entry (B6, B8/B9, T15-CORE, T16a) above. `git diff --stat 880994c16 HEAD -- src/app/golf/actions/round-recap.ts src/app/golf/actions/insight-celebration.ts src/lib/admin/observed-action.ts` = empty (zero changes to any of the 3 implicated files across this entire unit).
  - No occurrence this run of the third KNOWN class (jsdom `node_modules/node_modules/react` duplicate-module errors) — did not trigger this run; nothing to excuse.
- Confirmed (again, independently of the T15b entry above) `grep -rn "maybeCaptureRlsDenial" src/app/baseball src/lib/baseball src/app/lifting src/lib/lifting` = 0.
- **W15 status: Tasks 1-16 ALL DONE.** Final wrapped-action count: **424/424** (287 golf + 137 coachhelm, re-derived live by the foundation tripwire test on every run — not a hard-coded manifest). RLS-denial capture centralized in `fetch-all-rows.ts` + threaded through all 15 named call-site files/routes + 6 named ad-hoc-42501 files. Commit history for this unit: merge `67a6598ab`, then `feat(admin): W15 complete — final verification sweep` (EXECUTION_LOG entry, this commit).
...
- **Conflicts — 3 files, all real (not trivial), all resolved by union of behavior, none deleted either side's fix:**
  ...
  2. `src/app/admin/errors/[fingerprint]/page.tsx` — instrumentation decoded the fingerprint for display before calling the data layer; command-center restructured the page around an async `Body()` + `PanelBoundary`/`PanelNoData` pair. Kept command-center's `Body()`/PanelBoundary structure, feeding it instrumentation's raw (still-encoded) fingerprint plus the decoded value for the page heading.
  3. `src/lib/admin/data/triage.ts` (`isExpectedAuthNoise`) — both branches independently built noise-filtering for the same "you must be signed in" / "no active baseball team" auth control-flow, but with incompatible signatures: instrumentation's `mergeTriage` calls it with an `{title, message}` row; command-center's new `excludeAuthNoise()` (chained onto PostgREST queries in `overview.ts` + `admin-digest/route.ts`) calls it with a raw `message: string | null | undefined`. Unified into one function accepting either shape (`Pick<AppTriageEventRow,'title'|'message'> | string | null | undefined`), backed by command-center's broader `AUTH_NOISE_MESSAGE_PATTERNS`/`AUTH_NOISE_ILIKE_PATTERNS` exports (its "no active baseball team" pattern, without the "for this account" suffix, is a superset that still matches instrumentation's narrower test fixtures). Both pre-existing `describe('isExpectedAuthNoise', …)` blocks in `triage.test.ts` (8 assertions total, one per call shape) pass unchanged against the unified implementation.
- **Full gate (this unit):**
  - `npx tsc --noEmit` → exit 0, zero errors.
  - `npm run lint` → exit 0. 2275 pre-existing baseline warnings, **0 errors**, 0 new warnings.
  - `NODE_OPTIONS=--max-old-space-size=8192 npm test` (`vitest --project unit run`) → **4007 passed / 3 failed / 39 skipped** (4049 total, 374 passed / 1 failed / 1 skipped test files).
  - **3 failures, all in `src/lib/baseball/__tests__/program-type-nav-variants.test.ts`** ("coach nav ORDER differs between College and Showcase", "absent programType falls back to declaration order", "re-ordering never adds or drops entries") — confirmed pre-existing: `diff <(git show ac1124e34:<path>) <(cat <path>)` = empty, the test file is byte-identical to the pre-merge tip. Baseball nav-registry drift, out of Bridge scope per the hard rule (never touch baseball/lifting/crm).
  - **The 3 previously-tracked `revalidatePath`-outside-request-scope failures (`insight-celebration.test.ts` + `round-recap.test.ts`) now PASS** — re-ran both files in isolation (9/9 green), confirming the fix-B lane's mock landed on `feat/helm-bridge-command-center` and carried through the merge. Net: 6 pre-existing failures → 3.
- **Owner-decision items intentionally left untouched (not part of this fix sweep, need explicit sign-off):**
  1. `service_role` is the default read client across the entire admin data layer (`src/lib/admin/data/**`) — every query is gated by `requireSuperAdmin()`, but the client itself bypasses RLS everywhere, not just for the "view-as + integrity-check" surfaces where that's clearly required. Broader than strictly necessary; flagged, not narrowed.
  2. Baseball nav-variants test fixtures (`program-type-nav-variants.test.ts`) drifted against the live nav registry independent of any Bridge work — pre-existing red on `main` CI, out of scope for both draft PRs.
- Pushed `feat/helm-bridge-instrumentation` at merge commit `93da5023e` (+ the auto-regenerated `src/lib/types/database.ts` from the migration-changed pre-commit hook). Both PRs (#718, #727) commented with the sweep summary; neither marked ready-for-review — both remain drafts pending owner sign-off on the two items above.
