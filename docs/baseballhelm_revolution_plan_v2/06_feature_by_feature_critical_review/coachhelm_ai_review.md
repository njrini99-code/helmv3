...
V1 recognized `CoachHelm AI` as an important product area, but it was not always specific enough about role behavior, data ownership, edge cases, UI states, and implementation cutline.
...
Competitors researched: Teamworks analytics, Smartabase, generic SaaS AI summaries.
...
Phase recommendation: **Must build now as embedded**.
...
**Must build now as embedded.** Build only to the depth needed to support the V2 operating graph.
