...
| A production deploy or purchase through the Vercel MCP is refused | 7 deny rules (6 under the UUID spelling) | .claude/settings.json → permissions.deny | CONFIGURED — display-name and UUID spellings; NOT probed (the only probe is a real production deploy or a purchase); id stability UNVERIFIED |
