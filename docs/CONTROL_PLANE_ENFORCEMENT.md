...
| A write into the canonical checkout via Bash is refused | NONE | — | UNENFORCED — no hook on Bash resolves the canonical root; the Bash-matched hooks refuse command shapes, not writes by destination |
