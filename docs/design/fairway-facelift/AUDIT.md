...
| 8 | Thread pane does 2×N×R work per render | `summarizeReactions` runs twice per message per render, each a full scan of every reaction | done da4b8ba06: one useMemo grouping by message id |
...
| 12 | 200 message rows re-render on any pane state change | `MessageThreadPane.tsx` renders every message inline with no memo boundary | done c64ad5ad3: React.memo MessageRow with stable props; windowing skipped (flexible-height scroller under a stick-to-bottom pin, would need a dependency) |
| 13 | Group participants refetch on any inbox change | `FairwayMessages.tsx` effect depends on the whole `conversations` array | done 68be637f1: depends on a derived selectedConversationIsGroup boolean |
| 14 | Conversation rail re-splits on every keystroke | `MessageConversationRail.tsx` triage split is plain consts | done e2f1bebe5: useMemo on conversations, filter, now |
...
| M3 | Legacy `@/components/ui/drawer` (vaul, its own frost) inside Fairway: `pages/coachhelm/FairwayMyDevelopment.tsx`, `pages/rounds-new/FairwayCoursePicker.tsx`, and `app/golf/(dashboard)/dashboard/my-development/LogProgressButton.tsx` | A second bottom-sheet stack next to `fairway/overlays/Sheet`, with its own overlay z-index, blur and safe-area maths. | Migrate each to `Sheet` when its screen gets its mobile pass. Done for the development view (player-development.mobile.md #5: `LogProgressSheet` in `development-parts.tsx`, used by both `FairwayMyDevelopment` and the legacy `DevelopmentDrill`); still open for CoursePicker and `LogProgressButton` (the latter belongs to the pre-redesign route body). |
...
| M7 | Roster board on phone: `pages/roster/FairwayCoachRoster.tsx` reused MatrixBoard's literal hide-keys (`scor`/`composite`) for two columns and non-matching keys (`trendline`/`sig`) for two others to steer which columns survive below 940px | Column identity was encoded in key spelling; the phone row then carried four data columns in 289px (names cut at nine characters, "74.7↗" overlapping the trend cell, the Signal chip wrapping). | Roster mobile pass: the Trend column takes the literal `trend` key (hidden below 940px, 96px sparkline track on desktop) and the arrow rides inside the Avg cell on phone; Signal stays visible (`sig`). The primitives pass should give MatrixBoard a per-board `hideOnMobile` set and phone tracks (`minmax(0,2fr) auto auto` for identity boards) so the key trick can go. |
| M8 | Player home: `pages/dashboard/FairwayPlayerDashboard.tsx` hosts two legacy `components/golf/**` trees — `player-hub/HubInsightSignalCard` (the CoachHelm signal card: paragraph, tinted evidence panel, four stacked text actions, ~1,400 px on a phone) and `coachhelm/insights` `PlayerFocusAreas` (focus-area cards inside the page's Surface: card in card in card) | Two card vocabularies on one Fairway page. | Left in place by the player-home phone pass (signals and development belong to the CoachHelm lane); the Fairway replacements are `SignalCard`/InsightCard and the seam-row FocusAreaCard from `development.md`. |
| M9 | Player KPIs: below `md` the player home renders ONE `StatMatrix`, from `md` the eight `MetricCard`s (both in the DOM, CSS-gated) | Two renderings of the same eight numbers until the desktop player-home pass decides the primitive. | Temporary by design (no media-query flip on hydration). When the desktop pass lands, StatMatrix should win on both viewports and the MetricCard grid goes. |
| M10 | Player development: the live view at `/coachhelm?view=development` is the legacy `components/golf/coachhelm/home/DevelopmentDrill.tsx`, a verbatim port of `pages/coachhelm/FairwayMyDevelopment.tsx` (which the live route never mounts; only the fairway preview does). The port carried its own copies of the log progress drawer, the prescribed card and the overview instrument, and they had drifted (SourceChip only on the drill's card, `focusAreaCount` never forwarded, two toast wrappers). | Two implementations of one player screen; a mobile pass on the Fairway page changes nothing a player sees. | The shared pieces now live in `pages/coachhelm/development-parts.tsx` and both hosts import them; the drill's copies are gone. Follow-up for the CoachHelm lane: mount one host (the drill inside the stage, or the Fairway page) and delete the other. The completed and proposed sections are still rendered twice (once per host file), same markup. |
| M11 | Player development on phone (before this pass): six causal cards with nested readout blocks, three focus-area cards with two buttons each, three progress readout blocks, a legacy drawer; the completed row hid its own title (the status cluster took the width). | 15,300 px for one screen; the first action for an area was two buttons per card. | player-development.mobile.md RESULT: one StatMatrix, one InsetGroup of causal rows with a detail Sheet, one InsetGroup of focus-area rows with the full card in a Sheet, suggestions as rows, the completed row wraps its cluster below `sm`. Both hosts render the phone rows and the desktop cards CSS-gated (the hidden desktop cards still mount, including their PracticeRx self-fetch; a first-paint-safe unmount needs a settled viewport signal, not a media query). |
...
| Roster header grid | `FairwayCoachRoster.tsx` health card used `grid lg:grid-cols-[…]` with no track below `lg`; an auto track sizes to min-content, and PlayerIdentity's nowrap meta lines made it wider than a phone — the right edge of the card (attention copy, every "Add focus area") clipped. | `grid-cols-[minmax(0,1fr)]` at every width (roster mobile pass). |
| Roster phone sort | The four-option sort Segmented and the Export IconButton shared the toolbar's third line on phone; Export clipped at the viewport edge. | Below `sm` one "Sort · {label}" Button opens a matte Sheet (InsetGroup rows, `aria-pressed`, `selection` haptic) with Export as its last row; the Segmented/IconButton stay in the DOM in `hidden sm:contents` wrappers (pinned tests, desktop). |
| Qualifiers route skeleton | `app/golf/(dashboard)/dashboard/qualifiers/loading.tsx` still reserved the pre-facelift layout (status pills + search stacked, then a two-up card grid), so the load→content transition jumped from four cards to the seam list. | Skeleton redrawn to the live composition: bare toolbar line (search + three pills over a hairline) and one bordered Surface with two seam sections of rows. |
| Toolbar `viewToggle` hosting FilterPills | `pages/qualifiers/FairwayQualifiers.tsx` put its three status FilterPills in the Segmented-shaped `viewToggle` slot; on phone that is the right-pinned trailing cluster, so the pills read as orphaned from the search. `pages/tasks/FairwayTasks.tsx` kept the real Segmented there, which on phone became a lone right-pinned third line. | Qualifiers: pills into `filters` (left, scrolling line on phone). Tasks: Segmented desktop-only (`hidden sm:contents`), phone gets a "Status · {label}" control + matte Sheet in the filters line — the same pattern as the roster sort Sheet. Candidate primitive follow-up: a Toolbar-level phone fallback that renders any `viewToggle` Segmented as this Sheet automatically. |
...
Roster (2026-09-10, same rig): the player Sheet and the sort Sheet are matte (`Sheet` default material, no blur at any point), so neither has a settle gate to measure; their bodies are a goals line, one `FairwayIntentControl` and four `InsetGroup` rows. Nothing mounted inside them fetches. No frame-time table is recorded for them; the calendar table above is the reference for the `Sheet` primitive itself.
