...
**Why it matters:** This is the tenancy backbone. **Coach↔team is via `golf_team_coach_staff`, never `golf_coaches.team_id`** (`.claude/rules/golf-review.md`, `CLAUDE.md`). Getting this wrong doesn't just break a feature — it breaks the RLS model every other feature depends on for cross-tenant isolation (see §5).
