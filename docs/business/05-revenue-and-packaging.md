...
- `composeRoundReview`, `composeHeroNarrative`, and `composeCoachChat` are required to verify citations and regenerate once before falling back to template (per `.claude/rules/coachhelm-review.md`) — a PR that skips straight to template on any transient failure (not just budget exhaustion) is doing an unauthorized quality downgrade, not a budget-driven one, and should be flagged as a correctness issue even though it looks like the same code path.
