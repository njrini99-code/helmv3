...
All three share the same LLM budget/citation/fallback contract (`.claude/rules/coachhelm-review.md`).
...
- **Implementation:** `src/lib/coachhelm/v3/llm/budget.ts` — `checkBudget()` is called before every `compose()` invocation. It reads/upserts a `(coach_id, date)` row in `golf_coachhelm_llm_budget` (`budget_usd`, `spent_usd`), lazily seeding the day's budget from that coach's own `golf_coachhelm_settings.llm_budget_usd_per_day` the first time they spend on a given day. `recordSpend()` is the only thing that increments `spent_usd`, and is the caller's responsibility to invoke only on completed, billable calls (`00-business-context.md` §6, `05-revenue-and-packaging.md` §3a).
- **Per-coach, not per-team and not global.** The budget is resolved from the coach's own `golf_coachhelm_settings.llm_budget_usd_per_day` — that table is UNIQUE on `coach_id`, and the enforced cap is keyed `(coach_id, date)`. `golf_team_coachhelm_settings` is the team-level kill switch and carries no budget column, so there is no team-level budget to read. A coach with no `golf_team_coach_staff` row, or whose settings read fails, resolves to `0` and is denied; a coach who has never configured a budget gets `PLATFORM_DEFAULT_DAILY_BUDGET_USD`, which is still a real per-coach ceiling (`05-revenue-and-packaging.md` §3a).
