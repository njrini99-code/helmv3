...
- **the external review bot** — Use §2 as review context. When PRs touch `src/app/golf/actions/**` (golf_rounds writes), the RSC server→client boundary in golf dashboard routes, baseball migrations/RLS, or `src/app/golf/actions/insight-delivery.ts`, cross-check against these known production failures and against the **Hard rules** in the Review Gate rule packs (sport-prefixed table names, `SUPABASE_SERVICE_ROLE_KEY` restricted to admin server paths, and every `CREATE TABLE` shipping with RLS + a policy). Flag any PR that would reintroduce finding #1 (missing GRANT/RLS on `golf_rounds`), #2 (passing functions/non-plain objects to Client Components), or #3 (code referencing columns not present in the production DB).
