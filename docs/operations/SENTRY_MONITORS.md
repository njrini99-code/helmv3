...
| `9712025` | `451030` | Helm P1 — golf round submit/autosave terminal failure rate | `feature:golf_round_lifecycle` on `dataset: events_analytics_platform`, `aggregate: sum(helm.workflow.failure)` | 1h | 10 | 3 | 3 |

Verification: `GET /organizations/helm-xs/workflows/3937972/` →
`"detectorIds": ["7702315", "9711163", "9711158", "9711164", "9712025"]`.
...
- `feature:golf_round_lifecycle` / `sum(helm.workflow.failure)` (detector
  `9712025`): **this metric has zero data as of build time** — Phase C/D
  ...
  `feature:golf_round_lifecycle` is a **best guess** at the tag value Phase
...
|---|---|---|---|---|
| `3937972` | Helm — notify owner on metric monitor incidents | 60 min | email → user `4202373` (njrini99@gmail.com) | `7702315`, `9711163`, `9711158`, `9711164`, `9712025` (all P1) |
...
POST /api/0/organizations/helm-xs/alert-rules/
{
  ...
  "query": "feature:golf_round_lifecycle",
...
```
DELETE /api/0/organizations/helm-xs/detectors/<id>/      # 9711158, 9711163, 9711164, 9711165, 9712025, 9711171
