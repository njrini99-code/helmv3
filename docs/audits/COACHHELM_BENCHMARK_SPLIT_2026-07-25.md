...
- **The two systems don't just disagree in theory — they disagree on the same screen.** `StatsBento.tsx` (the Player Stats home view) renders a card literally labeled **"Standing"** whose headline text ("Strongest in X; leaking most in Y") comes from the *flat* S&W benchmark, while the card's own visual body (a Tour-value tick mark on a rail bar) comes from the *gender-aware* Standing benchmark — `src/components/golf/stats/spine-stage/StatsBento.tsx:134-135,253-262`. A coach doesn't need to navigate between two pages to see the contradiction; it's baked into one card.
