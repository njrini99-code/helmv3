...
Required contexts on `main` (verified = docs/CI_RUNBOOK.md, no drift): **CodeRabbit, CodeQL, `all` (ci.yml), `all` (review-gate.yml), Smoke checks** + 1 approving review + conversation resolution + linear history. Advisory-by-design (never chase as blockers): full Playwright E2E, Course-picker screenshots, BaseballHelm seeded smoke (advisory), CircleCI lighthouse-preview (red only because Vercel previews are paused via Ignored Build Step), CircleCI ios-compile, the external review bot.
...
| #572 | Fairway/the external review bot UI review (worktree helmv3-fix-wt), CONFLICTING; may overlap Lane B Fairway migration | `gh pr diff 572` vs Lane B before deciding |
...
5.3. Leave the advisory set advisory (Playwright full E2E, lighthouse-preview, ios-compile, seeded smoke, the external review bot) — the runbook is correct; lighthouse-preview self-resolves once Vercel previews deploy again.
