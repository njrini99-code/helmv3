...
**Node counts:** 24 features, 66 routes, 41 components, 27 apis, 91 actions, 50 services, 46 tests, 128 tables, 136 rpcs, 24 jobs, 11 invariants, 86 sentrySignals, 0 journeys.
**Edges:** 910 (merged; an edge with more than one evidence kind is a stronger claim).
**Journeys:** memory/journeys/ does not exist on this branch — journeys node list is empty, not a claim that no journeys exist.
...
- **RPCs:** `admin_auto_resolve_error_fingerprint`, `admin_mark_error_regressed`, `get_crm_coach_stage_history`, `get_crm_email_stats`, `get_crm_events_in_range`, `get_crm_funnel`, `get_crm_stage_ages`, `get_crm_time_to_open`, `get_crm_weekly_kpis`, `helm_debug_list_traces`, `recalculate_round_strokes_gained`, `refresh_player_stats_cache`
- **Test surfaces:** 2
...
- **Tables:** none
- **RPCs:** `admin_auto_resolve_error_fingerprint`
- **Test surfaces:** 2
- **Sentry/admin_events signals:** none
...
| `launchd:config/launchd/com.helm.bridge-rca-repair.plist` | launchd | `admin_selfheal` |
...
| `admin_reliability_collector` | `admin_incidents` | feature_doc_contract, import_graph |
...
| `admin_reliability_collector` | `shot_tracking` | import_graph (weak) |
...
| `admin_selfheal` | `admin_reliability_collector` | import_graph (weak) |
