...
| | |
|---|---|
| **CI seeds PRODUCTION on every PR** | `seed:baseball:ci` creates auth users and deletes `login_attempts` rows in the production project. It has always done this. It is now explicit in `package.json` (`--allow-prod`) rather than hidden behind a constant named "demo" — surfaced, not changed, because changing a working deployment behaviour unattended is not mine to do. |
| **Production Postgres was intermittently unreachable** | REST and auth answered 401 in ~0.1s while direct Postgres connections timed out on every attempt from 00:30 to 03:00, and CI's seed step got a Cloudflare 522 at 06:03 UTC. Reads as connection-pool exhaustion rather than an outage. It is why live `pg_policies` could not be confirmed. |
