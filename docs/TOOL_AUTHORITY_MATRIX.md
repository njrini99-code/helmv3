...
| `mcp__claude_ai_Vercel__*` | Vercel | AUTHORITY | yes | unknown (name not present) | no (not under this name) | partial (deploy_to_vercel denied) | STALE |
| `mcp__plugin_vercel_vercel__*` | Vercel | DEAD_DENIED | yes | no | no | DENIED (server-level) | STALE |
| `mcp__plugin_vercel-plugin_vercel__*` | Vercel | DEAD_DENIED | yes (plugin) | no | no | DENIED (server-level) | STALE |
| `mcp__fba2ada3-c190-4053-b91a-3e81f5296483__*` | Vercel | AUTHORITY_UUID_SPELLING | yes (account connector, no file in this repo) | yes | yes | partial (deploy, pause, purchases, deployment protection denied) | STALE |
