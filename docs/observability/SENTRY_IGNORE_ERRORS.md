...
| `'AuthRefreshDiscardedError'` | `@supabase/auth-js`'s own commit guard (`GoTrueClient._callRefreshToken`): a token refresh completed but storage changed under it mid-flight — a concurrent `signOut`, or another tab that rotated the refresh token first — so auth-js discards the rotated tokens. It is **returned as an `error` value, never thrown**; auth-js exports `isAuthRefreshDiscardedError` for callers and handles it internally in `__loadSession`. It becomes a Sentry *issue* only because `@sentry/core`'s Supabase auto-instrumentation (`instrumentAuthOperation`) captures any returned `{ error }` as `mechanism: { handled: false, type: 'auto.db.supabase.auth' }`. | None needed — the discard is the library's *correct* outcome, not a failure. Genuine session death still arrives as `AuthApiError` / `AuthSessionMissingError`, neither of which matches this pattern; the server-side round-lifecycle paths classify auth results independently via `isTransientAuthCheckFailure` (`src/app/golf/actions/golf.ts`), which this list does not touch. | A real auth outage produces a different error type (`AuthApiError`, `AuthRetryableFetchError`, a 5xx) and is unaffected. Residual risk is a **rate** signal, not a single event: this guard firing *often* would mean real refresh contention, and this filter would hide that from Sentry issues. First seen 2026-09-04 (`JAVASCRIPT-NEXTJS-RR`) at 1 event / 0 users over 7 days — re-measure before assuming that is still the rate. **Known gap this filter does not fix:** when another tab rotates first, `__loadSession`'s `stillStored.refresh_token === currentSession.refresh_token` guard fails and `getUser()` returns `user: null` for a still-signed-in user. Spurious multi-tab sign-outs on golf start there. |
