...
- **Mon/Alrt**: NO/UNKNOWN.
- **Live?**: NO.
- **Blind spot / action**: RESOLVED by a follow-up exhaustive pass (Findings §(e)): a tool-build failure (`"A plan we cannot build must never produce a Confirm button"`, `agent-tools.ts:163,240,269,549,573` — five sites, not two) does NOT reach Sentry/Bridge; it is purely the UI-visible `{status:'failed', message}`/progress-writer signal with zero server-side telemetry. Confirmed, not merely suspected. Fix: `logServerError` (warning) at each site.
...
- **Mon/Alrt**: NO/UNKNOWN.
- **Live?**: NO.
- **Blind spot / action**: RESOLVED by a follow-up read of `push.ts` directly (Findings §(d)): the token-read failure and total-rejection paths call `logServerError`; the per-device `invoke` failure branch, per-token catch, and outermost catch are `console.error`-only (Sentry via console safety net, no Bridge row). Separately, and higher severity: `push.ts:266-276`'s own comment (dated 2026-08-26) confirms the `send-fcm-push` Edge Function Android tokens route through **is not deployed in production** — every Android push 404s indefinitely and the dead token is never deactivated. Tracked as `RISK-041` in `docs/qa/helm-bug-risk-register.md`, a pre-existing acknowledged gap. `golf-message-fanout.ts:158` explicitly logs push failures as "(non-fatal)" — correct triage, not a gap.
