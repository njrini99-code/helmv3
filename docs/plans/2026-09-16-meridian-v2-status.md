...
- Task 20 carries the per-stage mount-compile split and a Web Worker
  proposal (not built), plus two output-identical compile passes that
  landed after it: the boundary SDF search (87f139c41) and the curvature
  and sky compiles on typed heights (4384e22cc) — hole 7's
  `assembleV2World` ~2.2 → ~1.4 s in Node, every compiled array
  digest-identical on holes 3, 7, 8 and 18.
