...
| 10 | Fairway edge treatment (crisp / soft / terrain-biased) | done (v1) | Crisp near bunkers/greens, soft elsewhere (`fairwayEdges` layer); terrain-biased since `edge-types-v2`: the lip follows the smoothed canonical slope across the edge (shoulder side darker, uphill side lighter), so a fairway riding a side slope sits on its landform; ribbon breaklines (renderer-redesign 3.7) give the mesh vertices along paths. No invented break: a flat mesh biases nothing (test) |
