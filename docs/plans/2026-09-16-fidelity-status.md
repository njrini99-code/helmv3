...
| 10 | Fairway edge treatment (crisp / soft / terrain-biased) | partial | Crisp near bunkers/greens, soft elsewhere (`fairwayEdges` layer, `edge-types-v1`); terrain-biased pending (breaklines) Ribbon breaklines now exist in the mesh (renderer-redesign 3.7); a terrain-biased fairway edge still needs an edge that follows a slope break, which no source provides. |
