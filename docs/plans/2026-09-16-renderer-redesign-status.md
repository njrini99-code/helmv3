...
| 3.7 / 6 | Terrain breaklines and multi-scale terrain | pending (source recompile) | Terrain compiler preserves feature edges; path cut/fill and water bank berms now exist as display offsets on the existing vertices (rows 12, 3.8), but breakline vertices along ribbon edges and banks need `compile-course-terrain.py` to constrain the triangulation with the context ribbons and a recompile of the 18-hole fixture from the USGS source; meso mounding stays out (no evidence, §6.3) |
