...
<!-- schema-drift-absent: golf_insight_feedback, golf_insight_weights, golf_player_insight_preferences, golf_review_insights, golf_validations -->
...
- `behavior-learner.ts` — Player behavior learning. FIXED 2026-09-22 (`agent/coachhelm-learning-cleanup`): the `'feedback'`-type interactions written by `rateInsight` are now bucketed by rating value (`classifyFeedback`, `NEGATIVE_RATINGS`), and `rateInsightImpl` (`insights.ts`) now records a real `insight_type` in its interaction metadata so `byInsightType` no longer collapses to `'unknown'`. `getPersonalizedThreshold` is now a real implementation (min-sample gate, ±25%-of-default clamp, falls back to the overall profile below the per-type sample floor) instead of a stub. In `orchestrator.ts`, `getLearnedPreferences()`'s result is now assigned and consulted (`computePersonalizedThresholds`), but only *applied* to `philosophy.declineThreshold`/`pressureGapThreshold` behind a new flag, `coachhelm_v2_alert_personalization` (default OFF in all environments; split 2026-09-23 from `coachhelm_learned_personalization`, which this consumer used to share with the unrelated v3 coach-weight read in `v3/ranking/score.ts` — see that file's own docblock) — with the flag off, the computed values are shadow-logged (`coachhelm.learned_personalization.shadow`) and never mutate `philosophy`, so `generateAlerts()` output is unchanged today. `cross-learner.ts`'s `findSimilarPlayers`/`transferLearning` remain correct but still have zero live callers outside their own test — untouched by this pass.
...
| `golf_coach_behavior_log` | ADDED 2026-07-25 — was going to back `coach-behavior.ts` (derivePreferences/prioritizeForCoach/recordAction/queryActions); that TS module was already removed before this pass (self-documented in `feedback/index.ts`'s header), leaving this table with **0 rows ever written in prod**. Table itself left untouched (no migration) per data-safety policy — orphaned, not actively read or written by any code path. |
...
- `outcome_status`: pending | improved | no_change | worsened | inconclusive
  - CORRECTED 2026-07-25, UPDATED 2026-09-22 (`agent/coachhelm-outcome-measure`): this field renders (the `OutcomeBadge` pill, `InsightCard.tsx:118-178`, rendered at `:466` default / `:605` hero). It was **100% NULL on all 548 prod rows** because the writer, `metricToRoundField`, only recognized v2-era metric names and none of the v3 generators' `MetricId`s matched. `backfillInsightOutcomes` (`v2/analytics/effectiveness-writer.ts`) now also resolves v3 `evidence.metric` ids via the v3 metric registry (`lookupMetricSource`/`averageInWindow`/`improvementSign`), keeping `metricToRoundField` as a fallback for legacy v2 names, leaves intentional-null metrics (`v3Source.kind === 'intentional-null'`) permanently unmeasured rather than guessing, and now also populates `outcome_metric_name`/`outcome_metric_before`/`outcome_metric_after`. A prod dry-run read (2026-09-22) found 449 eligible NULL rows, of which 116 resolve via the v3 registry and 0 via the legacy map — so most of the badge's remaining blankness on already-matured insights is the backfill's per-tick row cap draining over several cron ticks, not an unmapped-metric gap. Candidate fetch is paginated (`FETCH_PAGE_SIZE`/`MAX_FETCH_PAGES`, pre-filtered to measurable rows) to avoid starving on unmeasurable rows, mirroring the `causality-attribute` cron's precedent.
...
> - ~~**`HeroInsightCard.tsx`**'s exported wrapper~~ — REMOVED 2026-09-22 (`agent/coachhelm-learning-cleanup`), confirmed zero non-test importers by a fresh `git grep`. Every real hero-density card renders through the separate, duplicate local `HeroInsightCardInner` (`InsightCard.tsx:529-530`), which has no stagger/reveal logic and is unaffected.
> - **`FairwayPlayerCoachHelm.tsx`** (`fairway/pages/coachhelm/`) — documented in its own barrel as the player front door for `/dashboard/coachhelm`, but that route is actually served by `PlayerCoachHelmHome.tsx`. Still present (out of this pass's scope) — see barrel comment for the removal history of its sibling below.
...
> - ~~**`SectionBand`**~~ inside `FingerprintHero.tsx` (the coach per-player game page) — REMOVED 2026-09-22 (`agent/coachhelm-learning-cleanup`), confirmed zero importers anywhere. `MetricPill` from that same file remains live and untouched.
>
...
| `OutcomeBadge` (per-card outcome pill) | FIXED 2026-09-22, still partially DARK by design | Writer now resolves v3 `MetricId`s too (see DB Tables section above); 449 eligible NULL rows, 116 v3-resolvable, drained over several cron ticks (bounded per tick) — the rest are intentional-null metrics that stay unmeasured on purpose |
| `DrillAttachment.tsx` | DARK | Superseded by `PracticeRxPanel.tsx`; out of this pass's scope, not touched |
| `FairwayPlayerCoachHelm.tsx` | DARK | Route not rendered by it; superseded by `PlayerCoachHelmHome.tsx`; out of this pass's scope, not touched |
| ~~`FairwayMyDevelopment.tsx`~~ | REMOVED 2026-09-22 | Was dead (superseded by `DevelopmentDrill`); deleted along with its test, `git grep` confirmed zero non-test importers first |
| ~~`HeroInsightCard.tsx` wrapper (staggered reveal)~~ | REMOVED 2026-09-22 | Was dead; live hero cards render via duplicate local `HeroInsightCardInner` (`InsightCard.tsx:529-530`, unaffected) |
| ~~`SectionBand` (inside `FingerprintHero.tsx`)~~ | REMOVED 2026-09-22 | Was dead (zero importers); `MetricPill` from that file remains live and untouched |
| v2 coach-alert insight family (bubble_player, pattern_detected, streak, surge_player, plateau, tournament_pressure, closing_holes, par_3_issues, recurring_weakness, team_trend, scoring_decline) | LIVE-WRITTEN, DARK-READ | Actively generated daily (post-round trigger + `coachhelm-roster-sweep` cron), 100% excluded from every read surface — `engine_version` never stamped `v3` for this family, no v3 successor exists. Planned retirement: PR E (sequenced after `agent/coachhelm-outcomes` lands), not yet done as of 2026-09-22 |
| `BehaviorLearner.getLearnedPreferences()` in orchestrator | FIXED 2026-09-22 | Result now assigned (`computePersonalizedThresholds`); only *applied* to `philosophy` thresholds behind `coachhelm_v2_alert_personalization` (default OFF everywhere; own id since the 2026-09-23 flag split) — off state shadow-logs instead of mutating, so `generateAlerts()` output is unchanged while the flag is off |
| `BehaviorLearner.getPersonalizedThreshold` | FIXED 2026-09-22 | Real implementation (min-sample gate, ±25% clamp, profile-level fallback); wired per the row above, same flag gate |
...
| `baselineRegistry` (`v2/insights/baseline-registry.ts`) | DARK | No runtime caller anywhere — only its own test file invokes `get`/`tryGet`/`allKeys` (found during the N16 benchmark-label audit, 2026-09-23). Every generator that once might have used it (`putt-analytics.ts`) is deleted or rewired onto `cohort-baselines.ts` instead |
| `stats-insight-generator.ts`, `approach-analytics.ts` (`v2/mining`) | DARK | No importer outside `v2/mining` itself; not reachable from any live route or cron (N16 audit, 2026-09-23) |
| `team-pattern-generator.ts` (`v2/mining`) | LIVE | `generateTeamPatterns` called from `app/golf/actions/insights.ts:2488` inside `generateTeamInsight` — distinct from the two DARK files above despite living in the same directory |
