...
- Return visits are instant: `experimental.staleTimes.dynamic = 30`
  (next.config) keeps the route's RSC payload in the client router cache, and
  the dashboard shell `router.prefetch`es Calendar on idle
  (`useGolfSurfacePrewarm`). Server `revalidate = 30` is unchanged.
