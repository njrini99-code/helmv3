...
>
> The **AUTOGEN blocks are authoritative.** They are regenerated by
> `scripts/regen-docs.mjs` from `src/lib/types/database.ts`, which
> `npm run db:types` produces from the live database and `db:types:check`
> keeps honest. `AUTOGEN:tables` covers every table and view;
> `AUTOGEN:enums` covers every enum and its values.
>
> **The hand-written "Tables by Role & Feature" index below is NOT.** It was
> last hand-verified **2026-02-13** and by 2026-08-19 it named **20 tables
> that do not exist in production** — confirmed absent from `pg_class`,
> `pg_proc` and `pg_type` by direct query. They were listed in the same
> format as the real ones, with feature and purpose columns, so nothing
> distinguished them. Treat the index as a *navigational hint about intent*,
> and confirm any name against the AUTOGEN block before you query it.
>
> Two related traps, both since fixed, both worth knowing about:
>
> - The enum block reported **6 enums when the schema had 18** from at least
>   2026-02 until 2026-08-19. `extractEnums()` used one multi-line regex that
>   silently dropped every enum declared with a leading `|`, plus any enum
>   declared immediately after a match (a `lastIndex` swallow). It carried a
>   "DO NOT EDIT — regenerated" stamp the entire time, which is exactly what
>   made the short list persuasive. The parser is now line-wise and throws if
>   it extracts fewer enums than the block declares.
> - `docs:check` cannot catch either problem: it re-runs the generator and
>   diffs the output against this file, so it only ever compares the
>   generator to itself. `npm run docs:schema-drift` (CI: "Check knowledge
>   base against the schema") is the check that compares doc claims to the
>   actual schema.
>
...
- **Need a table name?** → `AUTOGEN:tables` below is the complete, verified list.
  The by-feature index is a browsing aid only — confirm the name there first.
- **Need column details?** → Go to `memory/context/golfhelm-database.md`
  (its `AUTOGEN:columns` block, not its narrative — that file says so itself)
- **Need enum values?** → `AUTOGEN:enums` at the bottom (18 enums, verified)
...
## Tables by Role & Feature (curated index — NOT complete, NOT verified; see AUTOGEN:tables)
...
**276 tables**, 9 views, 215 functions (source: `src/lib/types/database.ts`).
...
<details><summary>Database functions (215)</summary>
...
**18 enums** (source: `src/lib/types/database.ts`).
