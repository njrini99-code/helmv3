...
| 2026-08-27 17:50 | `dpl_3Um2qazUW844g8iYFyo2ftHX8naU` | `2fc0e4eac` | routine — owner-approved, over the weekly ceiling (see budget note) | YES — verified 2026-08-27, Vercel deployments API reports state READY, target production, gitCommitSha 2fc0e4eac0411191ed9d13278f218f24ca661934, aliased to helmsportslabs.com | Second promote of the day. Adds #1643 (shared Bridge row language, real incident descriptions, 24h→72h window, RCA link) and #1644 (Reliability/Jobs/Health on that language) on top of the 15:06 release. Built 9m26s, source `cli`, region iad1. Promoted from a worktree pinned at the exact merged SHA (`/private/tmp/helmv3-release`, detached at 2fc0e4eac, zero dirty files) — NOT the canonical checkout, whose local `main` still carries another session's unpushed commits. The first attempt failed with Vercel `Error: Not authorized` while `whoami`, `teams ls` and `project ls` all succeeded and the token had 8h left — a scope/refresh problem, not the guard and not an expired token (auth.json had been rewritten minutes earlier). Recorded because the obvious diagnosis was wrong twice. Owner ran the CLI: `guard-bash.sh` rule 12 blocks `vercel … --prod` for EVERY Claude Code session in this repo, not just one, and was NOT modified. Recorded within the hour. |
| 2026-08-27 21:36 (01:36 UTC 08-28) | `dpl_4frTFZRzBRKyZkDnjH7XSQfy2kQq` | `bdc09c915` | routine — owner-directed, over the weekly ceiling | health check passed | PR #1647: the self-healing loop closed (vocabulary in code, three-source triage as `npm run triage`, stages that heartbeat to `/admin/jobs`) plus the queue that engine produced — heartbeat 42501, PGRST116 at 8 call sites, the `replaceState` throttle guard, and four classification gaps. Deployed from a worktree pinned at this SHA, not the canonical checkout. `helmsportslabs.com` returned HTTP 200; `/api/health` returned `status: healthy`, `database: ok`, and echoed this exact deployment ID — so the SHA is confirmed by the running site, not inferred from timestamp proximity. **Fix verification is NOT claimed:** the two faults this release repairs last fired 5h and 12h BEFORE the promote, and zero error rows of any kind arrived in the first 10 minutes after it, so their silence is an absence of traffic rather than evidence of a fix. Their Sentry issues are deliberately left unresolved pending observation. 4th production deploy of ISO week 2026-W35 against a ceiling of 2 (`config/release-policy.yml`); owner-directed. |
