-- ============================================================================
-- Make putt_details and approach_miss_details inserts resilient in both RPCs.
--
-- Problem: If a detail INSERT fails (e.g. CHECK constraint violation), the
-- entire round transaction rolls back — losing holes, shots, everything.
-- Detail tables are supplemental analytics metadata and should never block
-- a round from saving.
--
-- Solution: Wrap each individual detail INSERT in a nested BEGIN/EXCEPTION
-- block (implicit savepoint). If a detail INSERT fails, only that record's
-- savepoint rolls back. Failed records are captured in a `warnings` array
-- returned alongside `success: true`.
-- ============================================================================
...
BEGIN
  ...
  -- Insert putt details with resilient savepoints
  ...
  -- Insert approach miss details with resilient savepoints
...
BEGIN
  ...
  -- Insert putt_details with resilient savepoints
  IF p_putt_details IS NOT NULL AND jsonb_array_length(p_putt_details) > 0 THEN
  ...
  -- Insert approach_miss_details with resilient savepoints
  IF p_approach_details IS NOT NULL AND jsonb_array_length(p_approach_details) > 0 THEN
