-- Migration: Fix statement timeout on round RPCs
--
-- Root cause: The `authenticated` role has statement_timeout=8s and lock_timeout=8s.
-- Both submit_round_atomic and save_partial_round_atomic use FOR UPDATE locking.
-- When auto-save holds the lock, submit waits — and the 8s timeout kills it.
-- SECURITY DEFINER changes permissions but does NOT override session-level timeouts.
--
-- Fix: Add SET statement_timeout and SET lock_timeout to both functions.
-- These override the session timeout only for the function call, then restore automatically.

-- 1. Fix submit_round_atomic (30s statement, 15s lock)
...
-- 2. Fix save_partial_round_atomic (20s statement, 10s lock)
