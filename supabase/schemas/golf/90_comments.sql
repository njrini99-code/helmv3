...
COMMENT ON COLUMN "public"."golf_player_focus_areas"."criteria" IS 'A8 slice 2: coach-authored (or engine-suggested) definitions of "done" for this focus area. Locked schema: {"entries": [{id, label, source: coach|engine, created_at, met, met_at}]}. Capped at ~10 entries, enforced in the action layer. NULL = no criteria set yet, or the write flag was off. Read-modify-write must compare a version marker (e.g. updated_at) before writing -- see the action layer''s compare-and-swap.';
