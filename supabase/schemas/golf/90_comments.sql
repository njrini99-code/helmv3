...
COMMENT ON TABLE "public"."golf_focus_area_criteria" IS 'A8 slice 2: coach-authored (or engine-suggested) "done" definitions for a focus area -- one row per criterion, not a jsonb column on golf_player_focus_areas. INSERT is coach-only; UPDATE is coach-only and column-restricted to (met, met_at, updated_at) via GRANT, so a coach can mark a criterion met/unmet but never rewrite its label or source. The cap of ~10 per focus area is enforced in the action layer. Gated behind config/feature-flags.yml''s coachhelm_focus_area_practice_log flag (default off) until this migration is applied in production.';

COMMENT ON TABLE "public"."golf_focus_area_practice_sessions" IS 'A8 slice 2: append-only log of actual practice completions against a golf_player_focus_areas row. One row per logged session (player or coach), never updated or deleted by application code -- see the REVOKE below, which grants authenticated INSERT and SELECT only. client_request_id + the UNIQUE(focus_area_id, client_request_id) constraint (golf_focus_area_practice_sessions_dedupe_key) make a double-submitted log entry (network retry, double-tap) a real no-op via ON CONFLICT DO NOTHING, without ever reading the table back to scan for a duplicate. Gated behind config/feature-flags.yml''s coachhelm_focus_area_practice_log flag (default off) until this migration is applied in production.';
