...
--
-- A btree turns each `max()` into an index-only scan. CONCURRENTLY so no
-- write on golf_shots is blocked while the index builds: this runs against a
-- live database with users on it, and golf_shots is the round-tracking hot
-- path.
--
