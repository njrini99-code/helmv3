-- Populate golf_player_stats_cache.driving_distance_average and
-- approach_proximity_average — both were NEVER written by any cache writer
-- (NULL for all players), yet read by player-fingerprint, team-category-insights,
-- stats-intelligence, and insights.ts. The shot data fully supports them:
--   driving_distance_average = AVG(shot_distance) over completed-round tee shots
--     with shot_distance > 0 (par-3 tee shots are stored as shot_type='approach',
--     so this is naturally par-4/5 only — matches the TS drivingDistanceAvg).
--   approach_proximity_average = AVG on-green approach finish in feet (holed = 0);
--     off-green misses record no proximity (matches the TS approachProximityAvg,
--     which only counts approaches that found the green).
...
BEGIN
  ...
  SET driving_distance_average = sub.dd,
      approach_proximity_average = sub.ap,
      updated_at = now()
  FROM (
    SELECT
      (SELECT ROUND(AVG(gs.shot_distance)::numeric, 1)
         FROM golf_shots gs
         JOIN golf_rounds r ON r.id = gs.round_id
        WHERE r.player_id = p_player_id AND r.status = 'completed'
          AND gs.shot_type = 'tee'
          AND gs.shot_distance IS NOT NULL AND gs.shot_distance > 0) AS dd,
      (SELECT ROUND(AVG(
                CASE WHEN gs.result = 'hole' THEN 0
                     WHEN gs.distance_unit_after = 'feet' THEN gs.distance_to_hole_after
                     ELSE gs.distance_to_hole_after * 3 END)::numeric, 1)
         FROM golf_shots gs
         JOIN golf_rounds r ON r.id = gs.round_id
        WHERE r.player_id = p_player_id AND r.status = 'completed'
          AND gs.shot_type = 'approach'
          AND gs.result IN ('green', 'hole')
          AND (gs.result = 'hole' OR gs.distance_to_hole_after IS NOT NULL)) AS ap
  ) sub
  WHERE psc.player_id = p_player_id;
END;
$function$;

-- Hook it into the explicit refresh path, next to update_player_putt_make_pct
-- (same freshness model as the putt bands). Body is verbatim from the current
-- definition with ONLY the one added PERFORM line.
...
BEGIN
  ...
  -- D1: populate driving_distance_average + approach_proximity_average (were
  -- never written -> NULL; consumed by fingerprint / category-insights / intelligence).
...
-- Backfill all existing cached players.
DO $$
DECLARE
  v_pid uuid;
BEGIN
