-- Migration: Add EXCEPTION handling + step tracking to round RPCs
--
-- Problem: When an RPC throws (timeout, trigger error, constraint violation),
-- PostgREST returns a generic error with no detail about WHICH step failed.
-- This makes debugging production failures nearly impossible.
--
-- Fix: Track v_current_step through the function. On EXCEPTION, return a
-- structured JSONB response with error code, message, step, and detail
-- instead of letting the error propagate.

-- 1. submit_round_atomic with exception handling
...
BEGIN
  ...
  -- Step 2: Lock round row
  v_current_step := 'lock_round';
  ...
  -- Step 3: Update round metadata + set status=completed
  v_current_step := 'update_round';
  ...
  -- Step 7: Insert putt details
  v_current_step := 'insert_putt_details';
  ...
  -- Step 8: Insert approach details
  v_current_step := 'insert_approach_details';
...
-- 2. save_partial_round_atomic with exception handling
