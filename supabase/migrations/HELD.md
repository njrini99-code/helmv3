...
| `20260917090000_helm_judgment_evaluations.sql` | **HOLD — R3, not yet reviewed** | Helm Judgment Layer (src/lib/ai/judgment/**): adds `helm_debug.judgment_evaluations` + `helm_debug.judgment_labels` and five service-role-only definer facades (`helm_debug_record_judgment(jsonb)`, `_label_judgment`, `_list_judgments`, `_get_judgment`, `_prune_judgments()`), same isolation pattern as 20260825200811/20260903180000 with the ACL tripwire. Applied and pgTAP-verified on the local stack only (`supabase/tests/rls/helm_judgment.sql`, 12/12). Not applied to production: it is R3 (new definer-rights functions + grants on a production schema) and `db-migration-reviewer` has not reviewed it. Until it ships, `src/lib/ai/judgment/persistence.ts` logs `helm.judgment.persist_failed` at warning and the judgment still returns — nothing user-facing depends on the row. After apply: regenerate `src/lib/types/database.ts` (the drift gate reads production) and wire `helm_debug_prune_judgments()` into /api/cron/db-observability-prune. | 2026-09-17 |
