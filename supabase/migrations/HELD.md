...
| `20260922120000_v3_standing_shot_metrics_all_shot_proximity.sql` | **APPLIED 2026-09-24 — hold discharged** | Applied via the `db-apply` workflow (run 35937766770, owner-authorized, `held_override` on this row) right after production deploy `6ee77e98e` was verified live, as the 2026-09-23 owner decision required. The workflow's own post-apply checks reported FAIL (ledger 0 rows, all five `-- VERIFY:` queries 0 rows), but a direct read of the live catalog minutes later shows every effect present: `golf_player_standing.basis` / `on_green_proximity_feet` / `layup_excluded_n`, `golf_player_standing_basis_check`, `refresh_player_standing_shot_metrics` containing `all_shot`, and ledger version `20260922120000`. Same false-FAIL pattern as the 2026-09-23 db-apply runs for `20260923180000` and `20260905090000`. Existing standings rows keep `basis = NULL` (the file writes no backfill) until the nightly `standing-refresh` cron recomputes them; `tour-basis.ts` withholds the Tour marker on NULL-basis rows, so nothing reads wrong in between. Historical reason for the hold: the all-shot proximity data and its companion UI (#1938/#1976) were to switch together. | 2026-09-23, applied 2026-09-24 |
