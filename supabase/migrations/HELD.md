...
| `20260922120000_v3_standing_shot_metrics_all_shot_proximity.sql` | **HOLD** | Owner decision 2026-09-23: apply right after the next production deploy, so the all-shot proximity data and its companion UI (#1938/#1976, not yet live) switch together. | 2026-09-23 |
| `20260924120000_golf_countable_round_stats_cache.sql` | **HOLD** | Owner decision OD-01 (2026-09-24): write and review the countable-round rule for the player stats cache and standings, but do not apply until the owner approves. Applying changes every cached golf average, best round, SG and pressure-gap figure at once (the Sep 17 37-stroke round and 4 hole-less rounds drop out), and it must be followed by the cache refresh in its header. NUM-24 (`20260924140000_golf_standing_pressure_gap_parity.sql`) builds on this file's Standing function body; apply it right after. | 2026-09-24 |
| `20260924140000_golf_standing_pressure_gap_parity.sql` | **HOLD** | NUM-24 (2026-09-24). Apply only AFTER `20260924120000_golf_countable_round_stats_cache.sql` (OD-01), in the same session; the file's DO block refuses to run unless the live `refresh_player_standing_round_metrics` body is OD-01's (md5 `d343740516f570d7a6a250273ce9bf11`). Makes Standing's pressure gap use the shared rule in `src/lib/golf/metrics/pressure-gap.ts`: 18-hole to-par basis and the legacy `'qualifying'` spelling. Read-only sizing: 0 `'qualifying'` rounds; 2 of 10 players move, by at most 0.33 strokes. One standing refresh after both files covers both. | 2026-09-24 |
| `20260924130000_golf_is_test_flag.sql` | **HOLD** | Owner decision OD-03 (2026-09-24): add the is_test hide flag file, owner approves apply. Additive (NOT NULL DEFAULT false on golf_rounds/teams/players/qualifiers) and inert until rows are flagged and read paths filter on it, which is a follow-up change. Any deletion of QA data stays a proposal. | 2026-09-24 |
| `20260925120000_golf_standing_team_pct_floor_5.sql` | **HOLD** | NUM-35 follow-up (2026-09-25): raises the team percentile floor (`v_min_team_n`) from 3 to 5 in `refresh_player_standing`, `refresh_player_standing_round_metrics` and `refresh_player_standing_shot_metrics`, matching `PERCENTILE_MIN_TEAM_N = 5` in `src/lib/coachhelm/v2/stats/percentiles.ts`. One constant and its COMMENT per function; nothing else changes. Apply only AFTER `20260924120000` (OD-01) and `20260924140000` (NUM-24), in the same session; the file's DO block refuses to run unless all three live bodies match the md5s in its header (round-metrics must be NUM-24's). Read-only sizing: 10 of 956 team percentiles (7 players, teams of 3-4) go NULL on the next refresh; percentiles.ts and StandingBar already gate at 5, other `team_pct` readers were not audited. Run the three refreshes in the file header after apply. Owner applies. | 2026-09-25 |
...
| `20260906115900_helm_debug_stat_statements_snapshot_min_exec.sql` + `20260906120010_helm_debug_db_statement_samples.sql` + `20260906120100_helm_debug_db_analysis_samples.sql` + `20260906120200_helm_debug_observability_retention_v3.sql` | **APPLIED — ledger-verified 2026-09-22** | **2026-09-22 ledger reconciliation:** a db-migration-reviewer session confirmed, read-only against the production ledger (`list_migrations`), that ledger rows `20260906115900`, `20260906120010`, `20260906120100`, and `20260906120200` (all four files in this row) are present in `supabase_migrations.schema_migrations` in production (project `qmnssrrolpinvwjjnufo`, `list_migrations`, re-confirmed independently 2026-09-22 via the read-only Supabase MCP), and that `20260903150000` (the agent_runs file, this file's own row above) is absent from that same ledger read. This reconciliation checked ledger presence only — it did not re-verify grants, ACLs, or behavior the way the earlier owner-authorized apply rows above did. Original HOLD reasoning, prepared before this row was known to be applied, follows for context. Prepared for the Database Plan track D5 (Database Tab: statement capture, index advice, bloat, coverage, drift, slow-statement Sentry issues) — see `docs/observability/DATABASE_TAB.md`. Same isolation pattern as every `helm_debug` migration before it: additive only (`create schema/table/index if not exists`, `create or replace function`), `helm_debug` already has no USAGE grant to anon/authenticated, every new/replaced facade is SECURITY DEFINER with a matching REVOKE and an in-file ACL tripwire, no direct table grant to any role. `…115900` adds `min_exec_ms` to the already-applied `helm_debug_stat_statements_snapshot`'s jsonb return (additive field, byte-identical signature — same non-negotiable v2/v3-signature discipline the retention migrations below restate). `…120000` adds `db_statement_samples`/`db_statement_alert_state` plus `record_db_statement_samples`/`helm_debug_read_statement_alert_state`/`helm_debug_read_db_statement_samples`, extending the existing `db-stat-delta` cron (still 15 min, no new schedule) to persist top-25-by-total and top-25-by-mean statement rankings from the SAME snapshot it already fetches, plus a once-per-UTC-day Sentry paging gate (`db:slow:<queryid>` fingerprint) for anything over a 500ms mean. `…120100` adds `db_analysis_samples` plus `helm_debug_db_analysis_snapshot`/`record_db_analysis_sample`/`helm_debug_read_db_analysis_samples`, extending the existing hourly `db-table-health` cron (no new schedule) with index-advisor suggestions (`index_advisor`/`hypopg`, capability-detected), unused indexes, bloat (`pgstattuple_approx`, capability-detected — never `CREATE EXTENSION` from this function), sequential-scan ratios, connection/lock counts, and two of the three RLS-coverage findings (`scripts/db/rls-coverage.mjs` computes the third — untested policies — standalone, since it needs repo file access a SQL function does not have). `…120200` `CREATE OR REPLACE`s `helm_debug_prune_observability` with the SAME byte-identical 4-argument signature v2 established (adding a parameter would create a second, ambiguous overload — see that migration's own header) to additionally prune the three new tables/state rows above at 30 days, fixed internal constants exactly like v2's own two. Apply in filename order — `…120200`'s body references `…120000`'s and `…120100`'s tables by name, so `CREATE FUNCTION` succeeds regardless of order but calling it before those two tables exist fails at execution time, the same caveat every prior HELD.md multi-file row in this series states for itself. Every cron route calling these RPCs (`db-stat-delta`, `db-table-health`) degrades cleanly while unapplied via the existing `isMigrationNotAppliedError` pattern — proven in this session by `src/test/api/cron/db-stat-delta.test.ts` and `db-table-health.test.ts`, which assert the pre-existing write (`db_stat_deltas`/`db_table_samples`) still succeeds even when the new RPCs 42883. `db-migration-reviewer` review has **not** been requested. No credential was reachable from this task worktree to verify production state directly (`.env.local` withheld) — this row is prepared, not verified applied. | 2026-09-06, ledger-verified 2026-09-22 |
| `20260905090000_baseball_camp_registrations_lifecycle_timestamps.sql` | **APPROVED — ready to apply (reviewed 2026-09-23)** | Adds `registered_at`/`attended_at` (timestamptz) to `baseball_camp_registrations`. `20260825224803_reconcile_baseball_active_read_contracts.sql`'s first block claimed these already existed live; the 2026-09-05 reconciliation's per-block live-column check found they do not — the one genuinely open gap out of that file's six blocks; re-confirmed absent 2026-09-23. **Hold discharged 2026-09-23**: the open question ("does the UI actually read/write these columns") is answered — `CampDetailClient.tsx` selects and `.order()`s by `registered_at` and `camps.ts`'s `checkInCampPlayer` writes `attended_at`, both live call sites, both currently failing. The file was widened past a bare `ADD COLUMN` to also backfill `registered_at` from `created_at` and default it going forward (ported from `20260825224803:23-31`) so existing rows sort correctly the moment the column exists, not just new ones. `db-migration-reviewer` review performed 2026-09-23: verdict CAUTION addressed — `SET LOCAL lock_timeout`, `-- VERIFY:`/`-- ROLLBACK:` headers added. **Not yet applied** — this row records reviewer approval, not a production apply; apply via `npm run db:apply` per `docs/operations/APPLY_PATH.md`. | 2026-09-05, approved 2026-09-23 |
| `20260905091000_baseball_timeline_event_acks_user_id_columns.sql` | **HOLD — SUPERSEDED by `20260923000000_baseball_timeline_event_acks_contract_repair.sql`** | Found by `db-drift.yml`'s daily production-drift check (5 consecutive failures, 2026-08-31 -> 2026-09-04). `src/app/baseball/actions/timeline-acks.ts` dual-writes `user_id`/`acknowledged_at` alongside the real `acked_by`/`acked_at` columns and then selects `acknowledged_at` back — but production has no `user_id`/`acknowledged_at` columns at all (confirmed live 2026-09-05). Held because whether this is actually causing acknowledgement writes to fail in production (versus PostgREST silently tolerating the extra keys) was not confirmed — see the file's header for what to check first. **2026-09-23: confirmed and corrected by independent `db-migration-reviewer` pass** — `query_logs` against `edge_logs` (24h window, project `qmnssrrolpinvwjjnufo`) shows exactly 40 requests to `/rest/v1/baseball_timeline_event_acks`, EVERY one a `GET` returning `400`/`PostgREST; error=42703`, and ZERO `POST`/`PATCH`/`DELETE` requests in that window — these are `getMyTimelineAcknowledgements`'s READ path (`.eq('user_id', ...)`), not the acknowledge/write path, which was not exercised in this window at all (consistent with the table's 0 rows). Had a write been attempted against the current schema it would fail differently, with PostgREST's `PGRST204` ("could not find column in schema cache") rather than Postgres's raw `42703` — both paths are broken by the same missing columns, and this migration's replacement closes both. This file stays HELD **as written** — do not apply it — because its `user_id uuid REFERENCES auth.users (id)` takes a lock on the shared `auth.users` table (also written by Golf signups) that a table with zero rows and an existing `acked_by` FK to the same target does not need. `20260923000000_baseball_timeline_event_acks_contract_repair.sql` is the reviewed replacement: same column additions, no second `auth.users` FK, an added visibility EXISTS gate on INSERT/UPDATE (an independent review pass 2026-09-23 found the first draft's actor-only WITH CHECK let a caller forge `team_id`/`player_id` to ack another team's event, or a player ack a `staff_only` event — closed by requiring the row's `(timeline_event_id, team_id, player_id)` to match a `baseball_player_timeline_events` row the caller can actually see), and the DELETE policy production has never had (confirmed via live `pg_policies` 2026-09-23 — there was no DELETE policy at all, so `withdrawTimelineAcknowledgement` has always been RLS-denied). **Separately, the ledger itself is unreliable for this table**: `list_migrations` carries a row at version `20260624000430` named `baseball_timeline_event_acks` — exactly this file's own version+name — yet NONE of the file's effects match the live catalog: no `user_id`/`acknowledged_at` columns (as this row already establishes), no `baseball_timeline_acks_*`-named policies (production's are `baseball_timeline_event_acks_*`), and no `baseball_timeline_event_acks_event_user_key` unique constraint (production's is `..._timeline_event_id_acked_by_key`). `list_migrations` also carries a SECOND, unexplained row at version `20260624202254` whose `name` is `20260624000430_baseball_timeline_event_acks` — i.e. it embeds this file's own original version+name inside its `name` field rather than being an independently named migration, which looks like a `supabase migration repair`-style ledger backfill for this same file rather than a second file ever running. `git log --follow` on `20260624000430_baseball_timeline_event_acks.sql` shows exactly one commit (2026-06-24) — the committed content has not changed since, so what's on disk today is what any of these ledger rows would have run if they ran this file's tracked text. That text opens with `CREATE TABLE IF NOT EXISTS baseball_timeline_event_acks (...)`, which — if production already had a table under this name in the older, incompatible shape at apply time — would explain the table-shape mismatch the same way `20260905092000`'s header documents for `baseball_pitch_events`/`baseball_workload_events` (a pre-existing table makes a `CREATE TABLE IF NOT EXISTS` a full no-op). It does NOT by itself explain why this file's separate, unconditional `ALTER TABLE ... ADD COLUMN IF NOT EXISTS user_id uuid` / `... acknowledged_at timestamptz` statements — which are not gated on the table not existing — never took effect live. Whether that's a second no-op this reconciliation hasn't traced, an out-of-band apply of different SQL than what's committed, or a ledger row stamped without ever executing this file's DDL was not resolved here; flagging it as open rather than asserting a specific cause. | 2026-09-05, superseded 2026-09-23 |
| `20260906120000_narrow_admin_event_purge_pg_cron.sql` | **APPLIED — ledger-verified 2026-09-22** | **2026-09-22 ledger reconciliation:** a db-migration-reviewer session confirmed, read-only against the production ledger (`list_migrations`), that ledger row `20260906120000` is present in `supabase_migrations.schema_migrations` in production (project `qmnssrrolpinvwjjnufo`, `list_migrations`, re-confirmed independently 2026-09-22 via the read-only Supabase MCP). This reconciliation checked ledger presence only — it did not re-verify grants, ACLs, or behavior the way the earlier owner-authorized apply rows above did. Original HOLD reasoning, prepared before this row was known to be applied, follows for context. 2026-09-06 cron consolidation: `src/app/api/cron/log-retention/route.ts` now purges `admin_events` (info/warning 90d, error/critical 13mo) and `admin_analytics_events` (180d, new in this same PR) in application code with proper batching and a counter in the job result. The pg_cron job `purge-admin-event-telemetry` (`20260703043000_admin_events_retention_pg_cron.sql`, live in production, `10 4 * * *`) currently duplicates both tables' purge with no batching and no severity awareness. This migration narrows it via `cron.schedule` (upserts by job name, so this is idempotent against the existing job) to a REDUNDANT FLOOR only: `admin_events` rows with `severity in ('info','warning')` older than 180 days, and nothing else — it stops touching `admin_analytics_events` entirely (log-retention owns that now) and never touches error/critical `admin_events` (log-retention's 13-month window for those is intentionally longer than any floor this job should apply). R3 per `memory/system/golfhelm-engineering-os.md`: privileged pg_cron schedule change against a live production job. Not applied by this PR; `db-migration-reviewer` review has not been requested. **Verification query for the owner to run after applying:** `select jobname, schedule, command from cron.job where jobname = 'purge-admin-event-telemetry';` — expect `schedule = '10 4 * * *'` and `command` containing only the single `admin_events` DELETE with the severity filter, no `admin_analytics_events` statement. | 2026-09-06, ledger-verified 2026-09-22 |
| `20260905092000_baseball_elite_stat_event_columns_gap.sql` | **APPROVED — ready to apply (reviewed 2026-09-23)** | Found by the same `db-drift.yml` failures: `baseball_pitch_events.batter_id`/`.pitch_type_classified`/`.is_called_strike`/`.count_state` and `baseball_workload_events.count`/`.high_intent_count` are all missing live, even though `20260624000080_baseball_elite_stat_event_model.sql` — the migration that defines them — **does have a ledger row** (`list_migrations` confirms it "applied"). Root cause: that migration's `CREATE TABLE IF NOT EXISTS` silently no-op'd against these two tables because they already existed under an older, incompatible column shape (live `baseball_pitch_events` still carries `pitch_type`/`called_strike`/`pitcher_id`, not the elite-model names) — the ledger records the statement ran, not that it did anything to these two tables. This file adds only the missing columns, additively, and does not attempt to reconcile the old and new column pairs — that split (coexist permanently vs. merge vs. retire the old ones) is a schema-design decision for whoever owns the elite stat event model. Re-confirmed missing live 2026-09-23. **Hold discharged 2026-09-23**: `db-migration-reviewer` review performed — `batter_id` changed from a bare `uuid` to `uuid REFERENCES public.baseball_players(id) ON DELETE SET NULL`, matching the elite-model source migration's own definition of this column (`20260624000080:189`) and every sibling player-identity FK on this table; `SET LOCAL lock_timeout` and `-- VERIFY:`/`-- ROLLBACK:` headers added. **Not yet applied** — apply via `npm run db:apply` per `docs/operations/APPLY_PATH.md`. | 2026-09-05, approved 2026-09-23 |
...
| `20260906140000_helm_jobs_pgmq_queues.sql` | **APPLIED — ledger-verified 2026-09-22** (also: applied 2026-09-09, owner-authorized — `helm_jobs_depth()` as originally written never ran; `returns table (queue …)` made `queue` an ambiguous PL/pgSQL OUT variable, so the body's unqualified `select queue … group by queue` failed with 42702 on every call. Fixed and applied as `20260909130000_helm_jobs_depth_qualify_queue.sql`, same signature and grants) | **2026-09-22 ledger reconciliation:** a db-migration-reviewer session confirmed, read-only against the production ledger (`list_migrations`), that ledger row `20260906140000` is present in `supabase_migrations.schema_migrations` in production (project `qmnssrrolpinvwjjnufo`, `list_migrations`, re-confirmed independently 2026-09-22 via the read-only Supabase MCP). This reconciliation checked ledger presence only — it did not re-verify grants, ACLs, or behavior the way the earlier owner-authorized apply rows above did. Original HOLD reasoning, prepared before this row was known to be applied, follows for context. Database Plan D6: enables the `pgmq` extension, creates three queues (`coachhelm_analysis`, `email_send`, `push_send`), a hand-owned `helm_jobs` schema (`dedupe_keys`, `dead_letters` — RLS enabled, no policies) and eight SECURITY DEFINER facades (`helm_jobs_enqueue`/`_read_batch`/`_ack`/`_fail`/`_depth`/`_requeue_dead_letter`/`_list_dead_letters`), each service-role-only with an ACL tripwire, same isolation pattern as `helm_debug` (20260825200811 and successors). R3: `CREATE EXTENSION`, a new schema, and grant changes. Daily reliability may prepare but must never apply; `db-migration-reviewer` review is mandatory first — **not yet performed**. Application code (`src/lib/jobs/enqueue.ts`, the round-submit and notification call sites) is fail-open while unapplied: `enqueueJob()` catches a missing-facade error (`42883`/`PGRST202`) and falls back to the existing inline/Inngest path unchanged, gated additionally behind `HELM_QUEUE_ENABLED` (default off, documented in `.env.example`) so the fallback is the default even after the migration ships. **Verify before applying** (owner, read-only): confirm `pgmq` is not already enabled under a different schema (`select * from pg_extension where extname = 'pgmq';`); if the CLI/connector role lacks `CREATE` on extensions, enable it via Dashboard -> Database -> Extensions first (this file's `create extension if not exists` becomes a no-op either way). **Verify after applying**: `select * from pgmq.metrics_all();` lists all three queues at length 0; `has_function_privilege('authenticated', 'public.helm_jobs_enqueue(text,jsonb,text)'::regprocedure, 'EXECUTE')` is false and the `service_role` equivalent is true, for all eight facades. pgTAP coverage: `supabase/tests/rls/helm_jobs_pgmq_queues.sql` (enqueue/dedupe/read/ack/fail/dead-letter contract plus anon/authenticated denial). **Not run locally** — this worktree has no Docker/local Supabase stack with the `pgmq` extension available to verify against; wired for CI's "Supabase lint + RLS tests" job to pick up on the next PR, which is the first real execution of this suite. | 2026-09-06, ledger-verified 2026-09-22 |
| `20260906141000_helm_jobs_pg_cron_consume_variant.sql` | **APPLIED — ledger-verified 2026-09-22 (function present, NOT scheduled — confirmed inert; Vercel Cron remains the live path)** | **2026-09-22 ledger reconciliation:** a db-migration-reviewer session confirmed, read-only against the production ledger (`list_migrations`), that ledger row `20260906141000` is present in `supabase_migrations.schema_migrations` in production (project `qmnssrrolpinvwjjnufo`, `list_migrations`, re-confirmed independently 2026-09-22 via the read-only Supabase MCP) — i.e. the function this migration creates now exists. **Resolved 2026-09-22:** this session queried `cron.job` directly (read-only, project `qmnssrrolpinvwjjnufo`) — the only row in the whole table is `purge-admin-event-telemetry` (20260906120000, above); no job exists for `/api/jobs/consume` or `helm_jobs_pg_cron_consume_tick`. The `select cron.schedule(...)` call in this file is still commented out, exactly as shipped — only the function was created, never scheduled. `vercel.json` still carries the Vercel Cron entry for `/api/jobs/consume` at `*/1 * * * *` (grep-confirmed). So: the default (Vercel Cron) path is the one live, the alternative pg_cron path is present but inert, and there is no double-processing today. Original HOLD reasoning, prepared before this row was known to be applied, follows for context. Database Plan D6 follow-on: an ALTERNATIVE scheduler for `/api/jobs/consume` using pg_cron + pg_net instead of Vercel Cron (the shipped default in this PR, `* * * * *` in `vercel.json`). Do not apply and schedule this alongside the Vercel Cron entry — both firing would double-process every tick. Requires the owner to first run `select vault.create_secret(<CRON_SECRET value>, 'helm_jobs_cron_secret');` (never inlined as a SQL literal, per this repo's hard constraint) and to enable `pg_cron`/`pg_net` if not already on. The `select cron.schedule(...)` call itself is commented out in the file and must be run explicitly, with the real production URL, only after the owner has decided to move off Vercel Cron for this route. `db-migration-reviewer` review is mandatory first — **not yet performed**. No pgTAP suite: this file adds one function whose only externally-observable effect (an HTTP call) cannot be asserted from pgTAP without a live pg_net responder; the ACL do-block above is the only automated check. | 2026-09-06, ledger-verified 2026-09-22 |
| `20260906142000_pgaudit_ddl_role_only.sql` | **APPLIED — ledger-verified 2026-09-22** | **2026-09-22 ledger reconciliation:** a db-migration-reviewer session confirmed, read-only against the production ledger (`list_migrations`), that ledger row `20260906142000` is present in `supabase_migrations.schema_migrations` in production (project `qmnssrrolpinvwjjnufo`, `list_migrations`, re-confirmed independently 2026-09-22 via the read-only Supabase MCP). This reconciliation checked ledger presence only — it did not re-verify grants, ACLs, or behavior the way the earlier owner-authorized apply rows above did. Original HOLD reasoning, prepared before this row was known to be applied, follows for context. Database Plan D7: enables `pgaudit` and sets `pgaudit.log = 'ddl, role'` at the `postgres` role level. Requires the extension to be enabled via Dashboard -> Database -> Extensions FIRST on most hosted Supabase projects (pgaudit needs `shared_preload_libraries`, which only the platform can set) — this file's `create extension if not exists` is a no-op check, not the actual enablement step. Scope is deliberately DDL + ROLE only, not read/write (see file header for why). `db-migration-reviewer` review is mandatory first — **not yet performed**. **Verify after applying**: `show pgaudit.log;` returns a value containing both `ddl` and `role`; a throwaway DDL statement in a scratch context produces an `AUDIT:` line in Dashboard -> Logs -> Postgres Logs within seconds. No pgTAP suite — a GUC value and a log-line side effect are not assertable from pgTAP; the file's own verify query above is the check. Depends on the Sentry log drain (docs/observability/SENTRY_SUPABASE_TRACING.md) being configured separately, by the owner, for these lines to reach Sentry — this migration only makes Postgres emit them. | 2026-09-06, ledger-verified 2026-09-22 |
| `20260922100000_course_geometry_storage_bucket.sql` | **APPLIED 2026-09-22 (owner-directed, before merge, via Supabase MCP)** | Course Geometry Factory v2, Phase 3 (D2: Supabase Storage). Adds one `storage.buckets` row (`course-geometry`, public read, 20 MB cap, `application/json` only) so a factory-published course package can be served from Storage instead of the app's own `public/course-geometry/` folder once a layout's `assetBaseUrl` points there — same shape as `20260613180000_course_images_storage.sql` (public bucket, no SELECT policy since a public bucket needs none and the app never lists it, only fetches exact hash-named paths). No write policy is added for `anon`/`authenticated` on purpose: `service_role` bypasses RLS/storage policies entirely, so omitting any INSERT/UPDATE/DELETE policy for those two roles is what "service-role-only writes" means here — publishing (`ship --approve`) is meant to upload through the service-role key from a trusted pipeline, never a player/coach session. Purely additive (`INSERT ... ON CONFLICT DO UPDATE` on `storage.buckets`, no table created, no existing policy touched); no golf_*/baseball_* object. `db-migration-reviewer` was dispatched twice this session. First pass: **CAUTION** — access model correct (no `storage.objects` policy exists that omits `bucket_id`, so nothing already-live widens this bucket), idempotent, MIME/size fine (largest real file 4.26 MB terrain, ~4.7x headroom under the 20 MB cap), no `.claude/rules/database.md` violation, golf-safety **SAFE** — conditioned on confirming live that no unscoped `storage.objects` policy exists: `SELECT policyname, cmd, roles, qual, with_check FROM pg_policies WHERE schemaname='storage' AND tablename='objects' AND coalesce(qual,'')||coalesce(with_check,'') NOT ILIKE '%bucket_id%';`. That query was run against production 2026-09-22 (by team-lead) and returned zero rows. Second pass, independently, against the live catalog: **VERDICT: CLEAR** — every live `storage.objects` policy is `TO authenticated` scoped to one named bucket by `bucket_id` equality, none catch-all, none granted to `anon`/`PUBLIC`; no bucket named/id'd `course-geometry` exists yet so the first apply is a plain insert; re-run resets the same three columns (idempotent); `.claude/rules/database.md` compliant; golf-safety **SAFE**. Both reviews flag the same forward-looking, non-blocking note for whoever writes the Storage uploader (none exists yet — `publish-course-assets.mts` only writes local files): pass `contentType: 'application/json'` explicitly (supabase-js defaults an unspecified upload to `text/plain;charset=UTF-8`, which this bucket's `allowed_mime_types` rejects), give hash-named files a long `cacheControl` (immutable), and upload `manifest.json` (the only file overwritten in place, not hash-named) with `upsert: true` and a short `cacheControl` so a stale manifest isn't served from CDN cache. **Not run locally** — this worktree has no Docker/local Supabase stack (`docker info` fails, `supabase` CLI not on PATH), so the bucket upsert has not been exercised against a rebuilt local database; the CLEAR verdict's checks were run against the live production catalog (read-only) instead. Peek'n Peak Upper is unaffected either way: its policy has no `assetBaseUrl`, so it keeps serving from `public/course-geometry/peek-n-peak-upper/`. **Applied 2026-09-22** at the owner's direction, from branch `agent/course-factory-phase0` ahead of merge: rehearsed first in a rolled-back transaction on production (bucket row created as specified, zero rows left afterwards), then the file body plus the `supabase_migrations.schema_migrations` row (`20260922100000`, `course_geometry_storage_bucket`) sent in one transaction the way `db:apply` does; the file's `-- VERIFY:` query returned 1 row and the ledger carries the version. Not rehearsed on a local Docker stack (additive, one row). | 2026-09-22 |
...
| `20260825222432` | `reconcile_baseball_timeline_ack_contract` | **CORRECTED 2026-09-23 — LOCAL-ONLY, NEVER RAN LIVE (was mislabeled "VERIFIED LIVE")** | `reconcile_*` name; `baseball_timeline_event_acks.team_id` and `.player_id` re-confirmed present via `list_tables` 2026-09-05 (alongside `acked_by`/`acked_at`) — and `user_id`/`acknowledged_at` re-confirmed ABSENT, consistent with the `20260905091000` HOLD row above. The prior "VERIFIED LIVE" label overstated that finding: `team_id`/`player_id`/`acked_by`/`acked_at` being present is evidence an EARLIER migration created them, not that THIS file ran — this file's own effect is a set of policies (`baseball_timeline_acks_select`/`_insert`/`_update`/`_delete`) and NOT NULL constraints, and neither is live. Live `pg_policies` read 2026-09-23 (project `qmnssrrolpinvwjjnufo`) shows production's actual policy names are `baseball_timeline_event_acks_select`/`_insert`/`_update` (note "_event_") with NO `_delete` policy at all — none of those names or that gap match what this file creates. Production's `unique (timeline_event_id, acked_by)` constraint is also unnamed the way this file's `DO` block checks for. This file is a from-scratch local-replay reconciliation only; its 2026-09-23 successor for the live gap is `20260923000000_baseball_timeline_event_acks_contract_repair.sql` (see the `20260905091000` row above). | Nothing to close in production; superseded for the live gap by `20260923000000`. |
| `20260825223149` | `reconcile_baseball_event_telemetry_production_contract` | **CORRECTED 2026-09-23 — MIXED, not purely LOCAL-ONLY** | `reconcile_*` name, same family as the row above; the prior "LOCAL-ONLY" label (given without independent column verification — this session's earlier re-verification pass was scoped to rows tagged "VERIFIED LIVE" or "UNRESOLVED," and this one carried neither) is only half right. This file is the FULLER version of the same fix `20260905092000_baseball_elite_stat_event_columns_gap.sql` now carries narrowly: both add `baseball_pitch_events.batter_id`/`.pitch_type_classified`/`.is_called_strike`/`.count_state` and `baseball_workload_events.count`/`.high_intent_count` — columns confirmed ABSENT from production via `information_schema.columns` 2026-09-23, the exact live gap `20260905092000` (now APPROVED, see its row above) closes. THIS file, applied as-is against production, would NOT be a no-op for those six columns. It genuinely IS local-only for its other two changes: `baseball_pitch_events.player_id` (confirmed PRESENT live 2026-09-23 — a pre-elite-model legacy column `20260624000080` never defined, so a fresh replay needs this file to add it, but production already has it) and its data backfills from the legacy `pitch_type`/`called_strike`/`pitcher_id`/`pitch_count`/`throw_count` columns into the new ones (backfill logic `20260905092000` deliberately omits — see that file's own header). Net: keep this file local-only (it is not queued for a production apply, and `20260905092000` is the reviewed path for the shared column gap), but do not cite it as evidence of a live no-op again. | Column gap closed by `20260905092000`; `player_id`/backfill parts remain local-only by design. |
