...
|---|---|---|---|
| `20260907160000_golf_team_chat_membership_management.sql` | **APPLIED — route other than `db:apply`** | Applied 2026-09-08 through the Supabase dashboard SQL editor, not `npm run db:apply`, because `db-apply` could not run it: the workflow's guard reported `0` pending while production was missing ten migrations (its awk parsed a `supabase migration list` row shape CLI 2.115.0 no longer prints — run 34178071502), and `apply.mjs` step (f) used `db push`, which applies EVERY pending migration. Both are fixed in this PR. The body sent was the reviewed file byte for byte plus the one ledger `insert` `db push` would have written, checksum-matched before running (19,941 bytes). Reviewed by `db-migration-reviewer` (CLEAR on the SQL). **Verified applied** 2026-09-08 against the live catalog: `golf_user_on_conversation_team` present, `prosecdef` true, `search_path=public, pg_temp`, ACL `postgres`/`authenticated`/`service_role` only — no `anon`, no `PUBLIC`; both recreated policies carry their new branches; ledger row present; all three of the file's own `-- VERIFY:` queries return 1 row. Applied OUT OF ORDER — 12 older migrations remain pending and were grepped for every object this file reads, with zero dependencies in either direction. | 2026-09-08 |
| `20260907120000_golf_participants_mute_columns.sql` | **APPLIED OUTSIDE THE LEDGER — needs reconciliation** | Not applied by this PR, and **not** a hold. Production already carries this file's entire effect — `golf_conversation_participants.notification_level` (NOT NULL) and `muted_until` (nullable) both exist, and the `golf_participants_notification_level_check` constraint exists — while `supabase_migrations.schema_migrations` has NO row for `20260907120000` (verified read-only 2026-09-08). Someone applied it by hand and skipped the ledger. This is the exact applied-≠-recorded drift `.claude/rules/database.md` warns about, and it is why the ledger `insert` is part of the apply body rather than left to a runner that is not in the loop. Consequence while unreconciled: the file reads as pending forever, so any `supabase db push` re-runs it (harmless — its DDL is `IF NOT EXISTS` and the constraint is `pg_constraint`-guarded — but it is live DDL for no reason). **Reconciliation is a one-row owner decision, not taken here:** `insert into supabase_migrations.schema_migrations (version, name) values ('20260907120000', 'golf_participants_mute_columns');` | 2026-09-08 |
