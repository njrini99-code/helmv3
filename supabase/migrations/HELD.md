...
| `20260905091000_baseball_timeline_event_acks_user_id_columns.sql` | **HOLD — SUPERSEDED by `20260923000000_baseball_timeline_event_acks_contract_repair.sql`** | Found by `db-drift.yml`'s daily production-drift check (5 consecutive failures, 2026-08-31 -> 2026-09-04). `src/app/baseball/actions/timeline-acks.ts` dual-writes `user_id`/`acknowledged_at` alongside the real `acked_by`/`acked_at` columns and then selects `acknowledged_at` back — but production has no `user_id`/`acknowledged_at` columns at all (confirmed live 2026-09-05). Held because whether this is actually causing acknowledgement writes to fail in production (versus PostgREST silently tolerating the extra keys) was not confirmed — see the file's header for what to check first. **2026-09-23: confirmed, not merely suspected** — Sentry/log evidence shows 36 occurrences of `column baseball_timeline_event_acks.user_id does not exist` in the preceding 24h, and the table carries 0 rows in production (every acknowledge attempt has failed outright). This file stays HELD **as written** — do not apply it — because its `user_id uuid REFERENCES auth.users (id)` takes a lock on the shared `auth.users` table (also written by Golf signups) that a table with zero rows and an existing `acked_by` FK to the same target does not need. `20260923000000_baseball_timeline_event_acks_contract_repair.sql` is the reviewed replacement: same column additions, no second `auth.users` FK, plus the INSERT/UPDATE policy tightening and the DELETE policy production has never had (confirmed via live `pg_policies` 2026-09-23 — there was no DELETE policy at all, so `withdrawTimelineAcknowledgement` has always been RLS-denied). | 2026-09-05, superseded 2026-09-23 |
