...
| `20260925120000_player_stats_sg_countable_rounds.sql` | **HOLD — owner applies** | Rewrites `update_player_stats_strokes_gained(uuid)` to average countable rounds only (the `round-countable.ts` rule) with 9-hole SG scaled x2 to per-18; clears a stale cache average when no countable round remains. Signature, SECURITY INVOKER, search_path and grants unchanged. Read-only dry-run 2026-09-25: 5 of the top 10 players move, 49ffe06d tee 1.98 -> 1.07, putting -2.93 -> -4.09. After applying, rerun the function per player (SQL in the file header). pgTAP: `supabase/tests/rls/golf_player_stats_sg_countable_rounds.sql`. | 2026-09-25 |
