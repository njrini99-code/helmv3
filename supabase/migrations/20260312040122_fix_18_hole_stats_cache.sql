-- ============================================================================
-- Migration: Align production golf stats cache with 18-hole scoring rules
-- ============================================================================
-- Changes:
--   1. update_player_stats_cache(): scoring_average and scoring_average_vs_par
--      now use completed 18-hole rounds only.
--   2. update_player_stats_cache_enhanced(): last_5_average and
--      last_10_average now use completed 18-hole rounds only.
--   3. refresh_player_stats_cache(): preserve driving_distance_avg during
--      full cache rebuilds.
--   4. Rebuild all golf stats cache rows to apply the corrected logic.
-- ============================================================================
...
BEGIN
  FOR v_player_id IN
    ...
    FROM (
      SELECT player_id FROM golf_rounds
      UNION
      SELECT player_id FROM golf_round_stats_cache
      UNION
      SELECT player_id FROM golf_player_stats_cache
    ) player_ids
    WHERE player_id IS NOT NULL
  LOOP
    PERFORM refresh_player_stats_cache(v_player_id);
  END LOOP;
END;
$$;
