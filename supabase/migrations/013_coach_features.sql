-- ============================================================================
-- Migration: 013_coach_features.sql
-- Purpose: Coach notes and calendar events
-- Consolidated from: 010_create_coach_features.sql
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_coach_notes_coach_player ON coach_notes(coach_id, player_id);
CREATE INDEX IF NOT EXISTS idx_coach_notes_created ON coach_notes(coach_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_coach_notes_tags ON coach_notes USING gin(tags) WHERE array_length(tags, 1) > 0;

-- Indexes for coach_calendar_events
CREATE INDEX IF NOT EXISTS idx_calendar_coach ON coach_calendar_events(coach_id);
CREATE INDEX IF NOT EXISTS idx_calendar_start ON coach_calendar_events(start_time);
CREATE INDEX IF NOT EXISTS idx_calendar_coach_range ON coach_calendar_events(coach_id, start_time, end_time);
CREATE INDEX IF NOT EXISTS idx_calendar_type ON coach_calendar_events(event_type) WHERE event_type IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_calendar_all_day ON coach_calendar_events(coach_id, start_time) WHERE is_all_day = TRUE;
...
-- Helper function: Get notes for a player
