...
create table if not exists helm_debug.db_error_events (
    ...
    bucket_started_at timestamptz not null default date_trunc(
        'hour', clock_timestamp()
    ),
    ...
    runtime text not null check (
        runtime in ('browser', 'node', 'edge', 'postgres')
    ),
    sport text,
    ...
    retryability text not null check (
        retryability in ('yes', 'no', 'conditional', 'unknown')
    ),
...
-- ACL-assertion tripwire, same pattern as
-- 20260826010000_helm_debug_retention.sql.
do $$
declare
...
revoke execute on function public.helm_debug_read_db_error_events(
    integer, timestamptz, text
)
from public, anon, authenticated;
grant execute on function public.helm_debug_read_db_error_events(
    integer, timestamptz, text
) to service_role;
