...
--   * RLS is ENABLED and anon is locked out on EVERY new V11 table.
--   * Player-owned health-adjacent rows (sessions / set results / soreness maps /
--     bodyweight / maxes / PRs) bind the player branch to the player's OWN
--     identity; staff reads of readiness-class rows require can_view_readiness
--     (the corrected V11 gate, NOT can_view_medical).
--   * Program/group/exercise/assignment WRITES are gated on can_manage_lifting.
--   * Materialized sessions + set results expose NO broad team-member read path
--     (a teammate can never read another player's loads).
--
...
SELECT ok(pg_temp.rls_on('baseball_lift_exercise_substitutions'),'RLS on baseball_lift_exercise_substitutions');
...
SELECT ok(pg_temp.rls_on('baseball_lift_set_results'),          'RLS on baseball_lift_set_results');
SELECT ok(pg_temp.rls_on('baseball_soreness_maps'),             'RLS on baseball_soreness_maps');
...
SELECT ok(pg_temp.rls_on('baseball_lift_import_runs'),          'RLS on baseball_lift_import_runs');
SELECT ok(pg_temp.rls_on('baseball_lift_import_rows'),          'RLS on baseball_lift_import_rows');
...
SELECT isnt(has_table_privilege('anon','public.baseball_lift_set_results','SELECT'), true, 'anon cannot SELECT baseball_lift_set_results');
SELECT isnt(has_table_privilege('anon','public.baseball_soreness_maps','SELECT'), true, 'anon cannot SELECT baseball_soreness_maps');
...
-- ============================================================================
-- 3. Set results — player owns; no peer read; INSERT self-bound.
-- ============================================================================
SELECT ok(pg_temp.polqual('baseball_lift_set_results','blsr_select') ~ 'get_my_baseball_player_id',
  'set_results SELECT binds player branch to own identity');
SELECT ok(pg_temp.polqual('baseball_lift_set_results','blsr_select') ~ 'can_manage_baseball_lift_group',
  'set_results SELECT non-owner path is staff-only');
SELECT ok(
  EXISTS (SELECT 1 FROM pg_policy p JOIN pg_class c ON c.oid=p.polrelid
          JOIN pg_namespace n ON n.oid=c.relnamespace
          WHERE n.nspname='public' AND c.relname='baseball_lift_set_results'
            AND p.polcmd='a'
            AND pg_get_expr(p.polwithcheck,p.polrelid) ~ 'get_my_baseball_player_id'),
  'set_results INSERT self-write is bound to the player''s own identity');

-- ============================================================================
-- 4. Soreness / bodyweight / availability — staff need can_view_readiness
--    (V11 corrected gate, NOT can_view_medical).
-- ============================================================================
SELECT ok(pg_temp.polqual('baseball_soreness_maps','bsm_select') ~ 'can_view_readiness',
  'soreness SELECT staff path requires can_view_readiness (not medical)');
SELECT ok(pg_temp.polqual('baseball_soreness_maps','bsm_select') ~ 'get_my_baseball_player_id',
  'soreness SELECT binds player branch to own identity');
