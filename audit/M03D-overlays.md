...
| Grip | `:48` 38×4px, `oklch(0.32 0.045 68/0.18)` | `Sheet.tsx:214` `h-1.5 w-10` = 6px×40px (`h-1.5`=0.375rem=6px, `w-10`=2.5rem=40px), `bg-border-strong` | Near-match, not a real gap: 40×6px actual vs 38×4px target is a 2px width / 2px height delta — scoring it a near-match rather than "off in height" so it doesn't read at the same severity as the genuinely-absent elements below. |
...
| Presence dot | `:96` green 11px dot, bottom-right of Alexis Bennett's avatar, 2px border | Not present (branch has no presence concept anywhere in messages) | **Flagging as `decision-needed` per the team lead's caution** — §24.5 bars inventing presence, but presence now appears in **two independent artboards** (`GroupDetails.dc.html:96` and the team lead's own `Main.dc.html` citation), which rules out a stray mock detail and strengthens the case that it's an intentional design requirement, not resolvable by declining it. No backing data exists to build it: `golf_conversation_participants` has `joined_at`, `last_read_at`, `muted_until`, `notification_level` (`database.ts:12308-12316`) and no `last_active_at`/presence column of any kind — this needs either a new column or a Realtime-presence channel. Recording for M00, not resolving or building it. |
...
- Evidence: `NewMessage.dc.html` is one unified sheet: `Cancel` / `New message` / `Start` header row (`:47-51`); removable recipient chips already selected (`:54-65`, pill with a 24px initials badge + name + × glyph); one focused search field with a green focus ring (`:68-72`); a `SEND TO EVERYONE` section containing a single **Team broadcast** row with a distinct icon and `"Everyone on Demo University Golf · 9"` subtitle (`:74-84`); a `PLAYERS` section of individually toggleable rows, each with a trailing selection ring (filled green check when selected, hollow outline when not) (`:86-128`); and a bottom pill CTA whose label changes with selection count — `"Start group · 2"` shown for 2 selected (`:130-135`). Note the header row already has its own `Start` affordance (`:50`) alongside the bottom pill (`:134`) — two separate commit controls in the same sheet; whether both ship, or one is the mock's placeholder for the other, is a composition question for the write phase, not resolved here. [`risk`/`proposal`, not `observed`: the artboard only shows the 2-selected state and the header `Start` label — what the bottom pill reads at exactly 1 selection (e.g. `"Open conversation"`) is inferred from §11.2's "primary action labels the actual operation," not read off any artboard state.]
...
- Evidence: `Actions.dc.html:40-56` and `Reactions.dc.html:76-92` are both full-width rows — 21px icon + 16px/500 text label per row, height 50-52px, vertically stacked, with an explicit divider line before Delete. Neither artboard renders a "close" control. Dismissal in both is a bottom-sheet with `border-radius: 1.75rem 1.75rem 0 0` (`Actions.dc.html:36`, `Reactions.dc.html:71`) and its own drag grip (`:37`, `:74`) — i.e. drag-to-dismiss via the grip, not scrim-tap: neither artboard renders a dim/scrim overlay div at all, just a reduced-opacity receded background (`Actions.dc.html:24` opacity 0.32, `Reactions.dc.html:36` opacity 0.34) behind the sheet itself. Current code, on both `main` (`MessageThreadPane.tsx:1114-1126`) and the branch (`:1497-1547`), renders `IconButton`-only glyphs (no text label at all) in a single horizontal `Inset` strip, plus an explicit `X` "Close" button that has no artboard counterpart.
...
- Gap: icon-only vs labeled-list is a different interaction pattern, not a styling delta — a user cannot read what a bare icon does without recognizing the glyph. The "X" close button is an extra element with no artboard counterpart — the artboards imply drag-dismiss via the grip and carry no close row at all — but it is not compensating for a missing scrim; neither artboard has a scrim to begin with (see F18's opacity-only receded background).
...
### F20 observed: `Group.dc.html`'s thread header is a §10.1 target that `main`'s header does not match [observed] [severity: med] — **advisory to M03B, `MessageThreadPane.tsx` is their lease**
- Evidence: `Group.dc.html` header region (lines 1-60 read): a 34px-avatar overlapping stack (`-12px` overlap) with a "+7" overflow badge, a title (16px/600) with a `"9 members"` subtitle (12px) directly beneath it (a live count, not static copy), and a trailing 44px info-circle icon button. Current `main` header (`MessageThreadPane.tsx:855-898`) shows a single static glyph/avatar (no stack), a title, and a subtitle that reads `"Group conversation"` (`:823`, generic copy) rather than a member count, with no trailing control of any kind.
- Spec: plan §10.1 (Group thread header).
- Gap: three concrete deltas — no avatar stack, generic subtitle text instead of a live count, and no trailing info affordance (which is also the identity/kebab entry point the branch adds — see priority-task section and F1). This falls inside `MessageThreadPane.tsx`, which §19.3 leases to M03B, not M03D — surfacing it here since it's squarely §10.1 (my section) but flagging it as advisory to M03B's lease rather than claiming the file.
...
- **New: `Group.dc.html`'s header (avatar stack, live member-count subtitle, trailing info control) has no counterpart in `main`'s thread header** (F20) — inside `MessageThreadPane.tsx`, leased to M03B; surfaced here as advisory since §10.1 is this lane's section.
