...
- [ ] **G-09** upload progress is fabricated [cross-lane + artboard confirmed] — G-09a landed; G-09b (real transport) is next
  ...
  - [ ] **G-09b** the real byte signal — XHR over `createSignedUploadUrl` replacing the fabricated constants, keeping `storage.upload()` as the fallback, and supplying the byte signal G-24's cancel needs. The PUT must set `Content-Type: resolvedMimeType` itself (`attachments.ts:261-272` explains why: an iOS camera capture arrives with a blank `file.type`, the SDK infers `application/octet-stream`, and the bucket's `allowed_mime_types` rejects it), and the fallback must trigger on a non-2xx status as well as on a throw
