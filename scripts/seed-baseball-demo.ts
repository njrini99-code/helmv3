...
async function main() {
  ...
  // already existed. The gap surfaced the first time it ran against a fresh
  ...
  // seed where that matters. The table carries `UNIQUE (team_id, coach_id)`
  // alongside its `id` primary key, and production's existing row has a RANDOM id
  // (uuid_generate_v4, since it was inserted by hand) rather than a deterministic
  // one. Conflicting on `id` would therefore try to INSERT a second row for the
  // same (team, coach) against production and fail the unique constraint — which
  // `playwright.yml` would hit on every push to `main`. Conflicting on the unique
  // pair updates the row that is actually there, whatever its id, and creates one
  // where there is none. No `id` is supplied at all, so the column default fills
  // it on insert and the existing value is preserved on update.
