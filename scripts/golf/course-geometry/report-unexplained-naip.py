...
SCENARIOS = [
    ('canopy', ('canopy',), 'every canopy pixel explained (a canopy pass re-run out to the hole bounds keeps fewer pixels but its gap-closing adds ground: measure-canopy-rerun.py gives the measured figure; a new package hash)'),
...
def main():
    ...
    lines += ['', f"Holes under the {GATE * 100:.0f} % gate: report {gate_counts['report']} of {len(holes_out)}; " + '; '.join(f"if {name} → {gate_counts[name]}" for name, _, _ in SCENARIOS) + '.', '',
              ...
              (f"- Canopy the pass never looked at: {beyond_m2 / 1e4:.1f} ha of the {canopy_m2 / 1e4:.1f} ha canopy inside the unexplained ground lies beyond the union of the canopy pass's per-hole boxes "
               ...
               "Re-running the pass out to the hole bounds would offer it to the same derivation (`measure-canopy-rerun.py` measures what its rules keep), at the cost of a new package hash (woods are package features)."),
