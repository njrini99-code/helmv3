async page => {
  ...
  for(const [width,height] of [[375,812],[390,844],[430,932],[320,568]]) {
    ...
    if(width===390) {
      ...
      await p.getByRole('button',{name:'Expand course view'}).click();
      await canvas.evaluate(el=>el.getContext('webgl').getExtension('WEBGL_lose_context').loseContext());
  ...
  await page.evaluate(value=>localStorage.setItem('golf-geometry-qa-terrain',JSON.stringify(value)),report);
}
