...
const modes = [ ['source', 320, 380], ['review', 320, 310], ['compact', 356, 160], ['strip', 48, 112] ] as const;
for (const hole of pilotPackage.holes) {
  ...
  for (const [mode, width, height] of modes) {
    ...
    if (mode === 'review') {
      const label = `<svg xmlns="http://www.w3.org/2000/svg" width="320" height="400"><rect width="320" height="400" fill="#f6f3ed"/><text x="8" y="20" font-family="sans-serif" font-size="14" fill="#18251f">${hole.key} · Par ${hole.par} · ${hole.scorecardYards ?? '—'} yd</text></svg>`;
      ...
      contactTiles.push({ input: tile, left: (index % 6) * 336 + 16, top: Math.floor(index / 6) * 416 + 72 });
    }
...
<h1>Cacapon · 18-hole geometry review</h1><p>© OpenStreetMap contributors · ODbL 1.0. Reviewed draft; independent course-familiar and current boundary review pending. NAIP 2022 comparison is a source agreement check; canopy and imagery age leave boundary accuracy unresolved. Hole 7 canopy groups use the native September 2024 NAIP crop; individual crowns and mowing surrounds are illustrative. Original-vector column is north up. Rebuild optional comparison images with source-overlay.py. No inferred cup or ball coordinates. Scorecard yardage does not resize a hole.</p>
...
// Actual-screen proof is served by browser.config.ts and captured by
// capture-screens.cjs; an SSR presentation board cannot verify interaction.
