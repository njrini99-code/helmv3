...
package_path, context_path, report_path, compiled, out = map(Path, sys.argv[1:])
...
lines = [f"# Outside-world §39 review prompts — {pkg['name']} ({pkg['siteId']})", '',
         ...
         f"Draft answers are derived from the data and are not a review; prompts marked **human** need the reviewer's eye "
         f"(imagery, course knowledge). Confirmed answers go into the layer's `review.notes`.", '',
...
for hole in sorted(pkg['holes'], key=lambda h: h['ordinal']):
    ...
    woods = playing.get('woods', 0) + ctx_areas.get('forest_mass', 0)
    ...
    answers = [
        ...
        (PROMPTS[1], 'derived', f"Largest non-playing areas inside the hole bounds: {top}. Unexplained (no source or rule): {pct(unexplained)} of {ha(total)}."),
        (PROMPTS[2], 'human', f"Woods on record: {ha(woods)} ({pct(woods / total)} of the hole bounds; package woods {ha(playing.get('woods', 0))}, OSM forest mass {ha(ctx_areas.get('forest_mass', 0))}). Density inside those polygons is seeded placement, not a source count."),
        ...
        (PROMPTS[9], 'human', f"Unexplained share {pct(unexplained)} ({'passes' if gate == 'pass' else 'fails'} the < {pct(GATE)} gate); the unexplained ground is painted as outer rough with terrain-following tone only, never a class."),
    ]
