...
export function FactoryLab() {
  ...
  // A selection renders before its effect clears the previous response.
  // Never pair the newly selected hole with the preceding hole's mesh.
  if (!loaded || loaded.hole.key !== holeKey) return <p role="status" className="p-6">Loading immutable factory bundle…</p>;
  ...
  return <main data-factory-bundle={bundle} data-package-hash={pkg.contentHash} data-mesh-hash={mesh.contentHash}
    ...
    <header className="flex flex-wrap items-center gap-3 p-3"><h1 className="font-semibold">{pkg.name}</h1>
      <label>Hole <NativeSelect aria-label="Physical hole" value={holeKey} onChange={event => updateHole(event.target.value)}>
        {manifest.holes.map(item => <option key={item.key} value={item.key}>{item.ordinal} · {item.key}</option>)}</NativeSelect></label>
...
if (mount) createRoot(mount).render(<FactoryLab />);
