async page => {
  const report={nativeDevice:false,viewport:{width:390,height:844,devicePixelRatio:2},holes:[],errors:[],source:'One compiled source revision; real course geometry; no player positions fabricated'};
  ...
  try {
    for(let hole=1;hole<=18;hole++) {
      ...
      await p.goto(`http://127.0.0.1:8768/?matrix=1&hole=${hole}`);
      ...
      for(const preset of ['Top','Terrain','Profile']) {
        ...
        await p.screenshot({path:`output/playwright/course-geometry/visual-system/course-matrix/${file}`});
