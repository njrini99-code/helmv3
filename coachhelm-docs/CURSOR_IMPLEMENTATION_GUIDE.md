...
This document explains how to implement the CoachHelm V2 Intelligence Engine in your GolfHelm project. The full specification is in `COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md`.
...
```
Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md carefully.
...
```
Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md — focus on "Part 1: Feature Extraction".
...
```
Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md — focus on "Part 2: Pattern Mining Engine".
...
```
Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md — focus on "Part 3: Predictive Engine".
...
```
Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md — focus on "Part 4: Adaptive Learning System".
...
```
Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md — focus on "Part 5: Reasoning Engine" and "Part 6: Natural Language Generation".
...
```
Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md — focus on "Part 7: Putting It All Together".
...
### CURSOR PROMPT — PHASE 9: Enable/Disable Feature

```
Read @coachhelm-docs/COACHHELM_DISABLE_FEATURE.md carefully.

Implement the master toggle that allows coaches and players to disable CoachHelm intelligence.

**Create these files:**

1. Database migration at `supabase/migrations/033_coachhelm_settings.sql`:
   - Add coachhelm_enabled columns to golf_coaches, golf_players, golf_teams
   - Create is_coachhelm_enabled() function
   - Add indexes

2. Add types to `src/lib/coachhelm/v2/types.ts`:
   - CoachHelmSettings interface
   - CoachHelmStatus interface

3. Create `src/hooks/coachhelm/useCoachHelmSettings.ts`:
   - Full hook implementation from the spec
   - enable(), disable(), enableForTeam(), disableForTeam() methods

4. Create `src/components/golf/coachhelm/settings/CoachHelmToggle.tsx`:
   - Toggle UI with glassmorphism design
   - Confirmation modal before disabling
   - Team controls for coaches

5. Create gating functions:
   - `src/lib/coachhelm/v2/gate.ts` — server-side checks
   - `src/hooks/coachhelm/useCoachHelmGate.ts` — client-side hook

6. Update orchestrator to check gate before running

**Key requirements:**
- Coaches can disable for themselves AND their team
- Players can only disable for themselves (unless team is disabled)
- When disabled, show helpful message instead of errors
- Graceful degradation throughout the UI
...
Phase 8: UI Integration ───────────────────────────► User-Facing
    │
    ▼
Phase 9: Enable/Disable Feature ───────────────────► User Control
```
...
src/hooks/coachhelm/
├── useCoachHelmSettings.ts               # Enable/disable hook
├── useCoachHelmGate.ts                   # Check if enabled
└── ...

supabase/migrations/
├── 032_coachhelm_v2_intelligence.sql     # V2 tables
└── 033_coachhelm_settings.sql            # Enable/disable columns
```
...
1. **Reference the spec frequently:**
   ```
   Read @coachhelm-docs/COACHHELM_V2_INTELLIGENCE_ENGINE_FULL.md section "Part 3: Predictive Engine"
   ```
...
1. These docs are now in your project at `coachhelm-docs/`
