...
live there today: `pre-commit` (remind you to run `npm run db:types` when a
migration is staged, and run `gitleaks protect --staged` if `gitleaks` is
...
npm run test:all     # Every Vitest project (RLS policies: npm run test:rls, pgTAP)
